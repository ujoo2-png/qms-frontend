...
const SB={
  ...
  async getMentions(){
    ...
    /* [v2.369] deleted_at IS NULL 필터 — 소프트 삭제된 항목 제외 */
    try{
  ...
  async addMention(row){
    ...
    /* [v2.369] SB mentions 테이블 실제 컬럼만 전송
  ...
  async updateMention(id,patch){
    /* [v2.369 PhaseA] status/channel/type/priority/pinned/reactions 포함 */
  ...
  async deleteMention(id){
    ...
    /* [v2.369] 소프트 삭제 — deleted_at 설정 (복구 가능) */
  ...
  async addEquip(row){
    ...
    /* [v2.369] 컬럼 자동 제거 루프 제거 — 에러 시 즉시 안내 */
    if(error){
