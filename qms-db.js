/* qms-db.js — DB 초기 데이터 + Supabase 객체 [v2.378] */
"use strict";
...
/* ── SB: Supabase DB 연동 헬퍼 ──
   ...
   [수정 이력]
   v2.378: SB 헬퍼 초기 구현 — getItems/getVendors/getInspections/getMentions 등
   v2.378: SB.uploadFile/deleteFile — Supabase Storage 연동
   v2.378: 일괄 버그 수정
...
const SB={
  ...
  /* ── [Phase 1 v2.378] 서버사이드 페이지네이션 헬퍼 ──
  ...
  async getInspections(type=null){
    ...
    /* [v2.378] _sbFetchAll: 1000건 제한 해제 */
  ...
  async addInspection(row){
    ...
    /* [v2.378] 허용 컬럼만 추출 — SB inspections 실제 컬럼만 포함
    ...
    const allowed={
      ...
      updated_at:  null,  /* [v2.378] 등록 시 수정일 비움 */
    };
    /* [v2.378] upsert 시도, 실패시 insert 폴백 */
  ...
  async getMentions(){
    ...
    /* [v2.378] deleted_at IS NULL 필터 — 소프트 삭제된 항목 제외 */
    try{
  ...
  async addMention(row){
    ...
    /* [v2.378] SB mentions 테이블 실제 컬럼만 전송
  ...
  async deleteMention(id){
    ...
    /* [v2.378] 소프트 삭제 — deleted_at 설정 (복구 가능) */
  ...
  async addUser(row){
    ...
    /* [v2.378] updated_at null로 명시 — 등록 시 수정일 비움 */
  ...
  async updateUser(id,patch){
    ...
    /* [v2.378] updated_at 오늘 날짜로 명시 — 수정 시만 기록 */
  ...
  /* [v2.378] deleteItems: 복수 일괄 삭제 (IN 쿼리로 1번에 처리) */
  ...
  /* [v2.378] deleteInspections: 검사 일괄 삭제 */
  ...
  /* [v2.378] vendors 컬럼 추가 SQL 안내 팝업 */
  _showVendorColSQL(){
  ...
  /* [v2.378] mentions 테이블 누락 컬럼 SQL 안내 */
  _showMentionColSQL(){
  ...
  async addEquip(row){
    ...
    /* [v2.378] 허용 컬럼만 추출 + 동적 컬럼 오류 제거 */
    /* [v2.378] 날짜 시리얼/객체 → YYYY-MM-DD 변환 */
    const _fmtD=(v)=>{
      if(!v&&v!==0) return null;
      if(v instanceof Date){const y=v.getUTCFullYear(),mo=v.getUTCMonth()+1,dy=v.getUTCDate();return `${y}-${String(mo).padStart(2,'0')}-${String(dy).padStart(2,'0')}`;}
      const s=String(v).trim();
...
const DB={
  ...
  users:[], /* [v2.378] 더미 삭제 — SB에서 로드 */
    inspections:[], /* [v2.378] 더미 제거 — SB에서 로드 */
  ...
  async addInspStd(row){
    ...
    const allowed={
      ...
      file_url:row.file_url||null,  /* [v2.378] 파일 URL */
    };
