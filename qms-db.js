/* qms-db.js — DB 초기 데이터 + Supabase 객체 [v2.381] */
"use strict";
...
/* ── SB: Supabase DB 연동 헬퍼 ──
   ...
   [수정 이력]
   v2.381: SB 헬퍼 초기 구현 — getItems/getVendors/getInspections/getMentions 등
   v2.381: SB.uploadFile/deleteFile — Supabase Storage 연동
   v2.381: 일괄 버그 수정
...
const SB={
  ...
  /* ── [Phase 1 v2.381] 서버사이드 페이지네이션 헬퍼 ──
  ...
  async getInspections(type=null){
    ...
    /* [v2.381] _sbFetchAll: 1000건 제한 해제 */
  ...
  async addInspection(row){
    ...
    /* [v2.381] 허용 컬럼만 추출 — SB inspections 실제 컬럼만 포함
    ...
    const allowed={
      ...
      updated_at:  null,  /* [v2.381] 등록 시 수정일 비움 */
    };
    /* [v2.381] upsert 시도, 실패시 insert 폴백 */
  ...
  async getMentions(){
    ...
    /* [v2.381] deleted_at IS NULL 필터 — 소프트 삭제된 항목 제외 */
    try{
  ...
  async addMention(row){
    ...
    /* [v2.381] SB mentions 테이블 실제 컬럼만 전송
    ...
    const insertRow={
      ...
      read:      row.read||false,
      file_url:  row.file_url||null,   /* [v2.381] 파일 첨부 */
    };
    /* created_at 있으면 포함 */
  ...
  async updateMention(id,patch){
    /* [v2.381] 실제 SB 컬럼만 업데이트 */
    if(!_sb){const m=DB.mentions.find(m=>m.id===id);if(m)Object.assign(m,patch);return {ok:true};}
    /* 허용 컬럼만 필터링 */
    const allowed={};
    const COLS=['from','to','to_list','text','message','dept','ref','reply_to','read','file_url','saved'];  /* [v2.381] */
  ...
  async deleteMention(id){
    ...
    /* [v2.381] 소프트 삭제 — deleted_at 설정 (복구 가능) */
  ...
  async addUser(row){
    ...
    /* [v2.381] updated_at null로 명시 — 등록 시 수정일 비움 */
  ...
  async updateUser(id,patch){
    ...
    /* [v2.381] updated_at 오늘 날짜로 명시 — 수정 시만 기록 */
  ...
  /* [v2.381] deleteItems: 복수 일괄 삭제 (IN 쿼리로 1번에 처리) */
  ...
  /* [v2.381] deleteInspections: 검사 일괄 삭제 */
  ...
  /* [v2.381] vendors 컬럼 추가 SQL 안내 팝업 */
  _showVendorColSQL(){
  ...
  /* [v2.381] mentions 테이블 누락 컬럼 SQL 안내 */
  _showMentionColSQL(){
  ...
  async addEquip(row){
    ...
    /* [v2.381] 허용 컬럼만 추출 + 동적 컬럼 오류 제거 */
    /* [v2.381] 날짜 시리얼/객체 → YYYY-MM-DD 변환 */
    const _fmtD=(v)=>{
      if(!v&&v!==0) return null;
      if(v instanceof Date){const y=v.getUTCFullYear(),mo=v.getUTCMonth()+1,dy=v.getUTCDate();return `${y}-${String(mo).padStart(2,'0')}-${String(dy).padStart(2,'0')}`;}
      const s=String(v).trim();
...
const DB={
  ...
  users:[], /* [v2.381] 더미 삭제 — SB에서 로드 */
    inspections:[], /* [v2.381] 더미 제거 — SB에서 로드 */
  ...
  async addInspStd(row){
    ...
    const allowed={
      ...
      file_url:row.file_url||null,  /* [v2.381] 파일 URL */
    };
