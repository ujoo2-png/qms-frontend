/* qms-excel.js — ExcelMgr + SearchPop [v2.316] */
"use strict";
...
const ExcelMgr={
  ...
  _schemas:{
    ...
    equip:{
      title:'계측기등록',
      cols:[
        /* [v2.316] 컬럼 순서 재정의 + model 추가 + 차기교정일 날짜만 */
  ...
  _renderPreview(raw,page){
    ...
    /* [v2.316] equip 전용 한글 별칭 매핑 (다른 schema와 충돌 방지) */
    if(page==='equip'){
      const equipAlias={
  ...
  async _register(page){
    ...
    const toAllowed=row=>{
      ...
      if(page==='equipment'||page==='equip') return{
        /* [v2.316] 전체 컬럼 명시 — maker/range/res/loc 누락 방지 */
        code:        row.code||'',
        name:        row.name||'',
