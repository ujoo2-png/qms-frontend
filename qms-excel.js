/* qms-excel.js — ExcelMgr + SearchPop [v2.327] */
"use strict";
...
const ExcelMgr={
  ...
  _schemas:{
    ...
    /* [v2.327] 계측기 업로드 양식 — 처음부터 새로 작성, 캐시 무효화 */
    equip:{
      title:'계측기_업로드양식',
      cols:[
  ...
  /* ── 양식 내려받기 ── */
  /* [v2.327] 파일명 생성 공통 함수 — 중복 로직 제거 */
  _fileName(title,suffix=''){
    /* [v2.327] 파일명: qms_제목_YYYY-MM-DD.xlsx (시각 제거) */
    const n=new Date();
  ...
  _renderPreview(raw,page){
    ...
    /* [v2.327] equip 전용 한글 별칭 매핑 (다른 schema와 충돌 방지) */
    if(page==='equip'){
      /* [v2.327] A_/B_/C_ 접두사 포함 매핑 + 기존 한글 그대로도 지원 */
      const equipAlias={
  ...
  async _register(page){
    ...
    const toAllowed=row=>{
      ...
      if(page==='equipment'||page==='equip') return{
        /* [v2.327] 전체 컬럼 명시 — maker/range/res/loc 누락 방지 */
        code:        row.code||'',
        name:        row.name||'',
