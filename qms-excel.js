...
/* ══ C: SPC ══ */
/* [v2.158] SPC 구버전 블록 제거 — qms-pages.js에 Supabase 실데이터 연동 버전으로 교체됨
   이 블록이 살아있으면 qms-pages.js의 새 버전을 덮어쓰는 문제가 있었음 */;

/* [v2.125] D: 부적합 심화 구버전 블록 완전 제거
   원인: 이 위치에 있던 구버전(프로토타입) 모듈
   (nc_8d/_8dDetail/_8dForm/nc_dispose/_disposeForm/nc_trend)이
   qms-pages.js 로드 이후 다시 로드되면서, 더미데이터(DB2.nc_dispose 등)를
   쓰는 이 구버전이 최신 실데이터(DB.disposals/DB.reports) 버전을
   전부 덮어쓰고 있었음(SQM v2.120, SearchPop v2.122와 동일한 패턴).
   증상: 더미 데이터가 삭제해도 계속 보이고, 인쇄 버튼이 안 보이고,
   등록해도 Toast만 뜨고 실제 저장되지 않음.
   해결: 구버전 블록 완전 삭제. 최신 구현은 qms-pages.js에 위치. */
...
var ExcelMgr=window.ExcelMgr={
  ...
  _schemas:{
    ...
    /* [v2.394] 계측기 업로드 양식 — 처음부터 새로 작성, 캐시 무효화 */
    equip:{
      title:'계측기_업로드양식',
      cols:[
  ...
  _fileName(title,suffix=''){
    /* [v2.394] 파일명: qms_제목_YYYY-MM-DD.xlsx */
    const n=new Date();
  ...
  _renderPreview(raw,page){
    ...
    /* [v2.394] equip 전용 한글 별칭 매핑 (다른 schema와 충돌 방지) */
    if(page==='equip'){
      /* [v2.394] A_/B_/C_ 접두사 포함 매핑 + 기존 한글 그대로도 지원 */
      const equipAlias={
  ...
  async _register(page){
    ...
    const toAllowed=row=>{
      ...
      if(page==='equipment'||page==='equip') return{
        /* [v2.394] 전체 컬럼 명시 — maker/range/res/loc 누락 방지 */
        code:        row.code||'',
        name:        row.name||'',
...
/* [v2.122] Search 팝업(F3) 구버전 SearchPop 객체 전체 제거
   원인: qms-core.js에 정상 SearchPop(open/search/reset/close 5개 메서드,
   sqm_plan 포함 최신 _cfg)이 있었으나, 이 파일이 나중 로드되며
   window.SearchPop=SearchPop으로 전역을 통째로 덮어씀.
   이 구버전은 open() 메서드만 있고 search/close/reset이 없어
   심사계획관리(sqm_plan)는 cfg 자체가 없어 항상 "Search를 사용할 수
   없습니다" 오류, 업체심사/평가/납품이력은 구버전 cfg로 동작(최신
   개선사항 미반영)되고 있었음. 해결: 이 블록 완전 제거. */
})();
