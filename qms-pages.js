/* qms-pages.js — Pages 페이지 렌더러 [v2.390] */
"use strict";
...
/* ── 홈 (메인화면) ──
   ...
   v2.390: C안 우측 패널 고정, 카드 높이 5배, stopPropagation 제거 */
home(){
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.390</div>
        </div>
...
/* ── 개인정보 수정 ── */
/* ── 멘션 답장 [v2.390→v2.390] ── */
  /* [v2.390 A안] _mentionReply 전체 재작성 — 문자열 연결 방식 (백틱 중첩 제거) */
...
async _mentionReplySend(parentId){
  /* [v2.390 PhaseB] thread_id 기반 SB 스레드 저장 */
...
async _approveUser(userId, username){
  ...
  /* [v2.390] 승인 후 화면 유지 — usermgmt 탭 그대로 */
...
/* 접근 권한 저장 (sessionStorage, v2.390) */
/* [v2.390] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
async _changePw(){
  ...
  /* [v2.390] admin 비밀번호 변경 — sessionStorage 저장 */
...
/* [v2.390] 설정 사용자관리 이름 클릭 → id로 사용자 찾아 수정 팝업 */
_uFormById(userId){
...
/* ── 품목 상세/등록/수정 팝업 ──
   [v2.390] 행 클릭 또는 품목코드 클릭 시 상세 팝업 열림 */
_iForm(row=null){
  const e=!!row;
...
/* [v2.390 수정] async로 전환 + SB.addVendor/updateVendor 연동
...
/* ── 사용자 ── */
async users(){
  /* [v2.390] 사원관리 — 인사 목록 (권한관리 없음, 활성사용자만) */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
...
/* [v2.390] A2: SHA-256 해시 저장 + password 컬럼 포함
...
async _uSave(id){
  ...
  if(id){
    ...
    /* [v2.390] admin 계정 이메일 수정 시 로그인창 footer 반영 */
  ...
  /* [v2.390] 저장 후 사원관리 이동 제거 — 사용자관리 탭 유지 */
  Modal.close();
  /* [v2.390] settings 사용자관리 탭 내 목록 갱신 — window.renderUserMgmt 사용 */
...
/* ── 임시비밀번호 발급 (관리자 전용, B2안) ──
   [v2.390] 관리자가 사용자 비밀번호를 임시값으로 초기화
...
/* ══════════════════════════════════════════════════════
   ...
   - v2.390: 구매검사(insp_pu), 외주검사(insp_ou) 추가
...
async _insp(type){
  ...
  /* [v2.390 복원] _sbFetchAll 전체 로드 후 로컬 필터 방식
     Phase1은 DOM 순서 문제로 무한 스피너 버그 발생 → 복원 */
  w.innerHTML='<div class="spin"></div>';
  /* [v2.390] 항상 SB에서 최신 데이터 로드 */
...
_ncForm(row=null){
  /* [v2.390] 부적합 등록/수정 폼 — 사내외/품목코드/고객 유형 추가 */
  const isEdit=!!row;
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.390] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.390] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.390] status null인 데이터 실시간 재계산 */
  /* [v2.390] status 항상 재계산 — DB 저장값 무시하고 next 날짜 기준으로 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.390] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
    ...
    {key:'file_url', label:'파일',       w:'64px', align:'center',  /* [v2.390] */
      render:(v,row)=>v
...
async _eqSave(orig){
  ...
  /* [v2.390 Phase1] 수정 시 변경 이력 자동 기록 */
  if(orig){
...
_eqDetail(row){
  /* [v2.390 Phase2] 탭 구조: 기본정보 / 교정이력 / 변경이력 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.390 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
_calForm(equip_code, calRow){
  ...
  Modal.open({title:isEdit?'교정이력 수정':'교정 등록',size:'mlg',
    body:'<div class="fg2">'
      ...
      +'<div class="fgroup" style="grid-column:1/-1">'   /* [v2.390] 파일 첨부 */
      +'<label class="fl">첨부파일</label>'
...
async _calSave(orig){
  /* [v2.390 Phase2] 교정 저장 + equipment.next 자동 갱신 */
  const g=id=>(document.getElementById(id)?.value||'').trim();
...
/* [v2.390 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.390 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async _mentionDetail(id){
  /* [v2.390] 멘션 상세 — 파일 미리보기+다운로드, 문자열 방식으로 안전하게 처리 */
...
/* ════════════════════════════════════════
   검사 고도화 — 검사 성적서 [v2.390]
   ════════════════════════════════════════ */
async insp_cert(){
  /* [v2.390] spin → 레이아웃 없을 때만, 있으면 데이터만 갱신 */
  const w=document.getElementById('pw');
...
/* ════════════════════════════════════════
   검사 고도화 — Hold 관리 [v2.390]
   ════════════════════════════════════════ */
async insp_hold(){
  /* [v2.390] SB holds 로드 — spin 최초1회 hasLayout */
  const w=document.getElementById('pw');
...
/* ════════════════════════════════════════
   검사 고도화 — 재검사 관리 [v2.390]
   ════════════════════════════════════════ */
async insp_reinsp(){
  /* [v2.390] SB reinspections 로드 — spin 최초1회 */
  const w=document.getElementById('pw');
...
/* ══════════════════════════════════════════
   설정 [v2.390] — 일반설정 / 사용자 등록 / SB 대시보드
   ══════════════════════════════════════════ */
async settings(){
  /* [v2.390] settings 복구 — 3탭: ⚙️일반설정 / 👥사용자관리 / 🔌SB대시보드 */
  const w=document.getElementById('pw');
...
async sysusers(){
  /* [v2.390] 사용자 등록 → settings() 로드 후 usermgmt 탭 즉시 활성화 */
  await Pages.settings();
  setTimeout(()=>{
...
/* 휴지통 — 삭제된 데이터 목록 [v2.390] */
async _renderTrash(){
  /* [v2.390] 휴지통 — 삭제된 데이터 복구 */
...
/* [v2.390] settings 공지/로고 — Cfg에 실제 구현, Pages에서 위임 */
...
Object.assign(Pages,{
async insp_std(){
  /* [v2.390] 검사 기준서 — SB 연동, hasLayout 패턴 */
  const w=document.getElementById('pw');
...
/* ── 검사 기준서 렌더 [v2.390] ── */
_inspStdRender(){
  /* [v2.390] 무한루프 수정 — w.innerHTML 최초1회, 테이블만 갱신 */
  const w=document.getElementById('pw');
  if(!w) return;
...
/* 심사 결과 메일 발송 [v2.390] — Gmail MCP 연동 */
...
Object.assign(Pages,{
nc_8d(){
  /* [v2.390] 8D Report — DB.reports 기반, tbar+F3+file_url */
  const w=document.getElementById('pw');
  const data=(DB.reports||DB.nc_8d||[]);  /* [v2.390] DB2 제거 */
  const total=data.length;
...
_8dRefresh(){
  ...
  const data=(DB.reports||DB.nc_8d||[]);  /* [v2.390] */
  ...
  Tbl.render({
    ...
    cols:[
      ...
      {key:'file_url',   label:'파일',       w:'60px',  /* [v2.390] */
        render:(v,row)=>v
          ?`<button class="btn bxs bblu" style="font-size:10px;padding:1px 7px"
...
nc_dispose(){
  /* [v2.390] 반품/폐기처리 — DB.disposals 기반, tbar+F3+F2+onRow */
  const w=document.getElementById('pw');
...
/* 반품/폐기 인쇄 [v2.390] */
_disposePrint(){
  /* [v2.390] Tbl 체크박스 class='rck', value=row.id */
...
const ExcelMgr={
  ...
  _schemas:{
    items:{
      ...
      /* [v2.390] 품목코드만 중복 확인, 필수값 외 빈칸 허용 */
      dupKey:'item_code',
      dupLabel:'품목코드',
    ...
    vendors:{
      ...
      /* [v2.390] 거래처명만 중복 확인, 필수값 외 빈칸 허용 */
      dupKey:'vendor_name',
      dupLabel:'거래처명',
      dupOnly:true,
      getData:()=>DB.vendors,
    ...
    insp_in:{
      ...
      /* [v2.390] 품목코드+거래처명 없으면 등록 안 됨, 동일시트 중복 허용 */
    ...
    insp_pr:{
      ...
      dupKey:'insp_no', dupLabel:'검사번호',
      allowFileDup:true,
      /* [v2.390] 품목코드+거래처명 필수, 동일시트 중복 허용 */
      dupKey:'insp_no', dupLabel:'검사번호',
      allowFileDup:true,
    ...
    nc:{
      /* [v2.390] 부적합 스키마 — 사내외/품목코드/수량/원인/조치/기한 추가 */
      title:'부적합관리',
      cols:[
  ...
  _parseFile(file,page){
    ...
    reader.onload=e=>{
      try{
        const wb=XLSX.read(e.target.result,{type:'array',cellDates:false});
        const ws=wb.Sheets[wb.SheetNames[0]];
        this._ws=ws; /* [v2.390] ws 저장 — 날짜 변환에 사용 */
        const raw=XLSX.utils.sheet_to_json(ws,{header:1,defval:''});
        /* [v2.390] 날짜 필드 변환 — 엑셀 시리얼/Date객체 → YYYY-MM-DD */
  ...
  _renderPreview(raw,page){
    ...
    /* ── 헤더행(0행) 처리 ──
       [v2.390 버그수정] 인덱스 기반 → 헤더 레이블 기반 파싱
  ...
  async _register(page){
    ...
    /* ── items 허용 컬럼 추출 함수 ── */
    const toAllowed=row=>{
      /* [v2.390] 테이블별 허용 컬럼만 추출 — SB schema 오류 방지 */
    ...
    /* [v2.390] equip: SB.addEquip 헬퍼 직접 호출 (검사5종 방식과 동일)
  ...
  /* 멀티시트 양식 다운로드
     [v2.390] pageFilter: 특정 시트만 포함 (null=전체) */
  ...
  /* 멀티시트 업로드 모달
     [v2.390] pageFilter: 특정 시트만 표시 (예: 'vendors', 'insp_in' 등)
             null이면 전체 8개 시트 표시 */
    ...
    reader.onload=async(ev)=>{
      try{
        ...
        for(const [sName,pKey] of Object.entries(SMAP)){
          ...
          /* [v2.390] raw:true 유지 — 셀 직접 접근으로 날짜 변환 처리 */
          const raw=XLSX.utils.sheet_to_json(ws,{header:1,defval:''});
  ...
  async _registerAll(){
    ...
    /* [v2.390] 엑셀 날짜 → YYYY-MM-DD (Date객체/시리얼/문자열 모두 처리) */
    const _toDate=(v)=>{
    ...
    const toFullRow=(pKey,r)=>{
      ...
      /* [v2.390] 검사5종 — SB.addInspection allowed와 동일 컬럼 */
      ...
      /* [v2.390] 계측기 — SB.addEquip allowed와 동일 컬럼 */
    ...
    for(const [pKey,result] of Object.entries(this._multiParsed)){
      ...
      /* [v2.390] 검사5종: SB.addInspection 직접 호출 (거래처 방식 — 컬럼 오류 자동 처리) */
      ...
      /* [v2.390] 계측기: SB.addEquip 직접 호출 (거래처 방식 — 컬럼 오류 자동 처리) */
        ...
        while(att<3&&!ok){
          try{
            ...
            if(!error){totalSuccess+=chunk.length;ok=true;}
              ...
              /* [v2.390] SAFE 저장 성공하면 colErrors 제거 — SQL 실행 후 팝업 억제 */
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.390] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.390 수정] 복원 시 저장된 사용자 정보로 표시 (admin 하드코딩 제거) */
      Auth._cur = cur;
      Auth._u   = u;
      ...
      /* [v2.390] 설정 메뉴: 세션 복원 시에도 admin만 표시 */
      ...
      /* [v2.390] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
