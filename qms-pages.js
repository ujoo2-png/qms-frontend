/* qms-pages.js — Pages 페이지 렌더러 [v2.379] */
"use strict";
...
/* ── 홈 (메인화면) ──
   ...
   v2.379: C안 우측 패널 고정, 카드 높이 5배, stopPropagation 제거 */
home(){
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.379</div>
        </div>
...
/* ── 개인정보 수정 ── */
/* ── 멘션 답장 [v2.379→v2.379] ── */
  /* [v2.379 A안] _mentionReply 전체 재작성 — 문자열 연결 방식 (백틱 중첩 제거) */
...
async _mentionReplySend(parentId){
  /* [v2.379 PhaseB] thread_id 기반 SB 스레드 저장 */
...
async _approveUser(userId, username){
  ...
  /* [v2.379] 승인 후 화면 유지 — usermgmt 탭 그대로 */
...
/* 접근 권한 저장 (sessionStorage, v2.379) */
/* [v2.379] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
async _changePw(){
  ...
  /* [v2.379] admin 비밀번호 변경 — sessionStorage 저장 */
...
/* [v2.379] 설정 사용자관리 이름 클릭 → id로 사용자 찾아 수정 팝업 */
_uFormById(userId){
...
/* ── 품목 상세/등록/수정 팝업 ──
   [v2.379] 행 클릭 또는 품목코드 클릭 시 상세 팝업 열림 */
_iForm(row=null){
  const e=!!row;
...
/* [v2.379 수정] async로 전환 + SB.addVendor/updateVendor 연동
...
/* ── 사용자 ── */
async users(){
  /* [v2.379] 사원관리 — 인사 목록 (권한관리 없음, 활성사용자만) */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
...
/* [v2.379] A2: SHA-256 해시 저장 + password 컬럼 포함
...
async _uSave(id){
  ...
  if(id){
    ...
    /* [v2.379] admin 계정 이메일 수정 시 로그인창 footer 반영 */
  ...
  /* [v2.379] 저장 후 사원관리 이동 제거 — 사용자관리 탭 유지 */
  Modal.close();
  /* [v2.379] settings 사용자관리 탭 내 목록 갱신 — window.renderUserMgmt 사용 */
...
/* ── 임시비밀번호 발급 (관리자 전용, B2안) ──
   [v2.379] 관리자가 사용자 비밀번호를 임시값으로 초기화
...
/* ══════════════════════════════════════════════════════
   ...
   - v2.379: 구매검사(insp_pu), 외주검사(insp_ou) 추가
...
async _insp(type){
  ...
  /* [v2.379 복원] _sbFetchAll 전체 로드 후 로컬 필터 방식
     Phase1은 DOM 순서 문제로 무한 스피너 버그 발생 → 복원 */
  w.innerHTML='<div class="spin"></div>';
  /* [v2.379] 항상 SB에서 최신 데이터 로드 */
...
_ncForm(row=null){
  /* [v2.379] 부적합 등록/수정 폼 — 사내외/품목코드/고객 유형 추가 */
  const isEdit=!!row;
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.379] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.379] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.379] status null인 데이터 실시간 재계산 */
  /* [v2.379] status 항상 재계산 — DB 저장값 무시하고 next 날짜 기준으로 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.379] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
async _eqSave(orig){
  ...
  /* [v2.379 Phase1] 수정 시 변경 이력 자동 기록 */
  if(orig){
...
_eqDetail(row){
  /* [v2.379 Phase2] 탭 구조: 기본정보 / 교정이력 / 변경이력 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.379 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
async _calSave(orig){
  /* [v2.379 Phase2] 교정 저장 + equipment.next 자동 갱신 */
  const g=id=>(document.getElementById(id)?.value||'').trim();
...
/* [v2.379 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.379 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
/* ════════════════════════════════════════
   검사 고도화 — 검사 성적서 [v2.379]
   ════════════════════════════════════════ */
async insp_cert(){
  /* [v2.379] spin → 레이아웃 없을 때만, 있으면 데이터만 갱신 */
  const w=document.getElementById('pw');
...
/* ════════════════════════════════════════
   검사 고도화 — Hold 관리 [v2.379]
   ════════════════════════════════════════ */
async insp_hold(){
  /* [v2.379] SB holds 로드 — spin 최초1회 hasLayout */
  const w=document.getElementById('pw');
...
/* ════════════════════════════════════════
   검사 고도화 — 재검사 관리 [v2.379]
   ════════════════════════════════════════ */
async insp_reinsp(){
  /* [v2.379] SB reinspections 로드 — spin 최초1회 */
  const w=document.getElementById('pw');
...
Object.assign(Pages,{
async insp_std(){
  /* [v2.379] 검사 기준서 — SB 연동, hasLayout 패턴 */
  const w=document.getElementById('pw');
...
/* ── 검사 기준서 렌더 [v2.379] ── */
_inspStdRender(){
  /* [v2.379] 무한루프 수정 — w.innerHTML 최초1회, 테이블만 갱신 */
  const w=document.getElementById('pw');
  if(!w) return;
...
/* 심사 결과 메일 발송 [v2.379] — Gmail MCP 연동 */
...
const ExcelMgr={
  ...
  _schemas:{
    ...
    nc:{
      /* [v2.379] 부적합 스키마 — 사내외/품목코드/수량/원인/조치/기한 추가 */
      title:'부적합관리',
      cols:[
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.379] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.379 수정] 복원 시 저장된 사용자 정보로 표시 (admin 하드코딩 제거) */
      Auth._cur = cur;
      Auth._u   = u;
      ...
      /* [v2.379] 설정 메뉴: 세션 복원 시에도 admin만 표시 */
      ...
      /* [v2.379] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
