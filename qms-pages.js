/* qms-pages.js — Pages 페이지 렌더러 [v2.319] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.319</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.319 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.319] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
async equip(){
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.319] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.319 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.319 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.319 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.319] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.319] 목록 헤더 */
    /* [v2.319] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.319 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.319 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.319 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.319] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.319 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
settings(){
  ...
  /* [v2.23] 3-A: 탭 구조 (기존 설정 + 사용자 관리)
     사용자 관리 탭은 관리자만 활성화 */
  /* [v2.319] renderTab — sbdash 전용 처리 포함 */
  const renderTab=(tab)=>{
  ...
  <div style="display:flex;gap:6px;margin-bottom:16px">
    ...
    <!-- [v2.319] SB 대시보드 탭 -->
    <!-- [v2.319] SB 대시보드 탭 - renderTab 후 패널 확실히 표시된 뒤 렌더 -->
...
/* [v2.319] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
  /* 최신 데이터 로드 */
...
/* [v2.319] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.319] SB 대시보드 — 탭 클릭 시 UI 표시, 새로고침 버튼으로 실제 데이터 조회 */
  const el=document.getElementById('sbDashContainer');
  if(!el){ Toast.show('설정 > SB 대시보드 탭을 먼저 열어주세요.','warn'); return; }
  /* 탭 클릭 시: 버튼만 있는 초기 화면 표시 */
  el.innerHTML=
    '<div style="padding:24px">'
    +'<div class="card" style="margin-bottom:16px">'
    +'<div class="ch"><div class="ct">🔌 Supabase 대시보드</div>'
    +'<span style="font-size:11px;color:'+(typeof _sb!=='undefined'&&_sb?'#22c55e':'#ef4444')+';font-weight:600">'
    +(typeof _sb!=='undefined'&&_sb?'● 연결됨':'● 미연결')+'</span>'
    +'</div>'
    +'<div style="text-align:center;padding:32px 16px">'
    +'<div style="font-size:40px;margin-bottom:12px">📊</div>'
    +'<div style="font-size:14px;font-weight:600;margin-bottom:8px">Supabase 사용량 대시보드</div>'
    +'<div style="font-size:12px;color:var(--tm);margin-bottom:20px">아래 버튼을 클릭하면 최신 데이터를 조회합니다.</div>'
    +'<button class="btn bpri" onclick="Pages._renderSbDashData()" style="font-size:13px;padding:10px 28px">🔄 데이터 불러오기</button>'
    +'</div>'
    +'</div>'
    +'</div>';
},

/* [v2.319] SB 대시보드 실제 데이터 조회 + 렌더 */
async _renderSbDashData(){
  const el=document.getElementById('sbDashContainer');
  if(!el){ Toast.show('SB 대시보드를 먼저 열어주세요.','warn'); return; }

  /* 로딩 표시 */
  el.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm);font-size:13px">🔄 데이터 조회 중...</div>';
  ...
  const AVG={equipment:800,calibrations:500,users:400,mentions:600,
    items:300,vendors:350,nonconformances:1000,cars:800,documents:500,equipment_logs:300};

  let counts={}, totalRows=0;
  if(typeof _sb!=='undefined'&&_sb){
    await Promise.all(tables.map(async t=>{
      try{
        const {count,error}=await _sb.from(t).select('*',{count:'exact',head:true});
        counts[t]=error?0:(count||0); totalRows+=counts[t];
      }catch(e){counts[t]=0;}
    }));
  }
  ...
  /* 도넛 SVG */
  function donut(pct,color,val,sub){
    ...
    return '<svg width="120" height="120" viewBox="0 0 120 120">'
      ...
      +'<text x="'+CX+'" y="'+(CY-6)+'" text-anchor="middle" font-size="14" font-weight="700" fill="#1e293b">'+val+'</text>'
      +'<text x="'+CX+'" y="'+(CY+12)+'" text-anchor="middle" font-size="10" fill="#64748b">'+sub+'</text>'
      +'<text x="'+CX+'" y="'+(CY+26)+'" text-anchor="middle" font-size="10" fill="'+color+'" font-weight="600">'+pct+'%</text>'
      +'</svg>';
  }
  ...
  el.innerHTML=
    ...
    /* 도넛 3개 */
    +'<div class="card" style="margin-bottom:14px">'
    +'<div class="ch"><div class="ct">📊 사용량 현황</div>'
    +'<button class="btn bsm bout" onclick="Pages._renderSbDashData()">🔄 새로고침</button>'
    +'</div>'
    +'<div style="display:flex;justify-content:space-around;padding:12px 0">'
    +'<div style="text-align:center">'
      +'<div style="font-size:11px;font-weight:600;color:var(--tm);margin-bottom:6px">📊 DB 사용량</div>'
      +donut(DB_PCT,DB_PCT>80?'#ef4444':DB_PCT>60?'#f59e0b':'#3b82f6',DB_USED+'MB','/'+DB_MAX+'MB')
    +'</div>'
    +'<div style="text-align:center">'
      +'<div style="font-size:11px;font-weight:600;color:var(--tm);margin-bottom:6px">💾 Storage</div>'
      +donut(ST_PCT,'#10b981',ST_USED+'MB','/'+ST_MAX+'MB')
    +'</div>'
    +'<div style="text-align:center">'
      +'<div style="font-size:11px;font-weight:600;color:var(--tm);margin-bottom:6px">⏰ 비활성 방지</div>'
      +donut(ALIVE_PCT,aliveColor,daysLeft+'일','/ 7일')
    +'</div>'
    +'</div></div>'
    /* 테이블별 현황 */
    +'<div class="card" style="margin-bottom:14px">'
    +'<div class="ch"><div class="ct">📋 테이블별 현황</div>'
    +'<span style="font-size:11px;color:var(--tm)">총 '+totalRows.toLocaleString()+'행</span>'
    +'</div>'
    +'<table style="width:100%;border-collapse:collapse;font-size:12px">'
    +'<thead><tr style="background:var(--bg2)">'
    +'<th style="padding:6px 10px;text-align:left">테이블</th>'
    +'<th style="padding:6px 10px;text-align:right">행 수</th>'
    +'<th style="padding:6px 10px;text-align:right">예상크기</th>'
    +'<th style="padding:6px 10px">비율</th>'
    +'</tr></thead><tbody>'
    +tables.map(t=>{
      const cnt=counts[t]||0;
      const kb=Math.round(cnt*(AVG[t]||400)/1024);
      const pct2=estBytes>0?Math.min(100,Math.round(cnt*(AVG[t]||400)/estBytes*100)):0;
      return '<tr style="border-bottom:1px solid var(--bd)">'
        +'<td style="padding:6px 10px">'+H.e(LABELS[t]||t)+'</td>'
        +'<td style="padding:6px 10px;text-align:right;font-weight:600">'+cnt.toLocaleString()+'</td>'
        +'<td style="padding:6px 10px;text-align:right;color:var(--tm)">'+kb+'KB</td>'
        +'<td style="padding:6px 10px"><div style="background:var(--bd);border-radius:4px;height:6px;overflow:hidden">'
        +'<div style="background:#3b82f6;height:6px;width:'+pct2+'%;border-radius:4px"></div>'
        +'</div></td></tr>';
    }).join('')
    +'</tbody></table></div>'
    /* keepalive */
    ...
    +'Supabase 무료 플랜: <strong>7일 미접속 시 일시정지</strong><br>'
    +'마지막 keepalive: <strong id="sbLastPing">'+lastPing+'</strong>'
    +'</div>'
    +'<div style="display:flex;gap:8px;align-items:center">'
    +'<button class="btn bpri bsm" onclick="Pages._sbKeepAlive()">🔄 keepalive 전송</button>'
    +'<span style="font-size:11px;color:'+aliveColor+';font-weight:600">'
    +(daysLeft<=2?'⚠️ 곧 비활성 위험':daysLeft<=4?'⚠️ 주의 필요':'✅ 정상')
    +'</span>'
    +'</div></div>'
    +'</div>';
},
...
async _sbKeepAlive(){
  /* [v2.319] Supabase keepalive — 7일 비활성 방지 */
  try{
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.319] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.319] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
