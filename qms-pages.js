/* qms-pages.js — Pages 페이지 렌더러 [v2.334] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.334</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.334 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.334] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.334] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.334] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.334] status null인 데이터 실시간 재계산 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.334] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.334 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.334 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.334 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.334] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.334] 목록 헤더 */
    /* [v2.334] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.334 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.334 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.334 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.334] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.334 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
settings(){
  ...
  /* [v2.23] 3-A: 탭 구조 (기존 설정 + 사용자 관리)
     사용자 관리 탭은 관리자만 활성화 */
  /* [v2.334] renderTab — sbdash 전용 처리 포함 */
  const renderTab=(tab)=>{
    ...
    /* [v2.334] usermgmt: 클릭 시 isAdmin 재확인 후 동적 렌더 */
    if(tab==='usermgmt'){
      const el=document.getElementById('usermgmtBody');
      if(!el) return;
      const _isAdmin=Auth._u?.role==='admin';
      if(_isAdmin){
        el.innerHTML=renderUserMgmt()+renderPermTable();
      } else {
        el.innerHTML='<div class="card" style="text-align:center;padding:40px">'
          ...
          +'<div style="color:var(--tm);font-size:13px">관리자만 접근할 수 있습니다.</div></div>';
      }
    }
    /* [v2.334] sbdash: 클릭 시 항상 렌더 */
    if(tab==='sbdash') Pages._renderSbDash();
  };
  ...
  <!-- 사용자 관리 탭 -->
  <!-- [v2.334] usermgmt 패널 — renderTab 시점에 동적 렌더 -->
  <div class="stab-pane" data-tab="usermgmt" style="display:none">
    <div id="usermgmtBody"><div style="text-align:center;padding:20px;color:var(--tm)">로딩 중...</div></div>
  </div>
  <!-- [v2.334] SB 대시보드 탭 패널 -->
...
`;
  ...
  /* [v2.334] renderTab 전역 등록 */
  window.renderTab=renderTab;
  /* [v2.334] 현재 활성 탭이 sbdash면 즉시 렌더 */
  const _activeSbTab=document.querySelector('.stab-btn.on[data-tab="sbdash"]');
  if(_activeSbTab) setTimeout(()=>Pages._renderSbDash(),50);
},
/* [v2.334] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  /* [v2.334] 사용자 등록/권한 관리 — 완전 독립 구현 */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
...
_renderSysUsers(w=null){
  /* [v2.334] 사용자 등록/권한 관리 독립 렌더 */
  /* [v2.334] w가 있으면 레이아웃 생성 */
  if(w){
    w.innerHTML=`<div class="ph"><div><div class="ptit">👤 사용자 등록 / 권한 관리</div></div><div class="pac"><button class="btn bpri btn-f2" onclick="Pages._addUser()">+ 사용자 등록 <span class="kbd">F2</span></button></div></div><div id="sysUserBody"></div>`;
  }
...
/* [v2.334] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.334] SB 대시보드 — 전체 테이블 누락 없이 */
  /* [v2.334] sbDashContainer — general 탭이 숨겨져도 getElementById는 작동 */
  ...
  el.innerHTML=
    ...
    /* [v2.334] 도넛 차트 5개 — DB/Storage/Egress/전체행/비활성방지 */
...
async _sbKeepAlive(){
  /* [v2.334] Supabase keepalive — 7일 비활성 방지 */
  try{
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.334] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.334] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
