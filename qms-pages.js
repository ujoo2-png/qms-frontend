/* qms-pages.js — Pages 페이지 렌더러 [v2.317] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.317</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.317 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.317] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
async equip(){
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.317] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.317 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.317 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.317 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.317] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.317] 목록 헤더 */
    /* [v2.317] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.317 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.317 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.317 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.317] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.317 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
settings(){
  ...
  <div style="display:flex;gap:6px;margin-bottom:16px">
    ...
    <!-- [v2.317] SB 대시보드 탭 -->
    <!-- [v2.317] SB 대시보드 탭 - renderTab 후 패널 확실히 표시된 뒤 렌더 -->
    <button class="btn stab-btn bout" data-tab="sbdash"
      onclick="renderTab('sbdash');Pages._sbDashOpen()"
...
/* [v2.317] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
  /* 최신 데이터 로드 */
...
/* [v2.317] SB 대시보드 — 사용량 + 비활성 방지 */
_sbDashOpen(){
  /* [v2.317] sbdash 패널이 DOM에 완전히 표시된 후 렌더
     renderTab()이 display:block으로 전환하지만 동기적으로 바로 getElementById 하면
     template literal 재렌더 타이밍에 el이 null일 수 있음 → requestAnimationFrame 사용 */
...
async _renderSbDash(){
  /* [v2.317] SB 대시보드 — 파이차트 + 테이블 + keepalive */
...
async _sbKeepAlive(){
  /* [v2.317] Supabase keepalive — 7일 비활성 방지 */
  try{
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.317] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.317] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
