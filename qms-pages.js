/* qms-pages.js — Pages 페이지 렌더러 [v2.51]
...
async doc_approval(){
  ...
  try{
    /* [v2.51] users 테이블(설정→사용자관리)에서 현재 로그인 사용자 매칭
       Auth._u = users row 전체. id/name/username 순으로 매칭 */
    var users=await SB.getUsers();
    var meId=null;

    /* [v2.51] 사용자 매칭 강화 — 설정→사용자관리 users 테이블과 연동
...
/* ══════════════════════════════════════════════════
   D4: 개정 이력 타임라인 [v2.51]
   ══════════════════════════════════════════════════ */
/* [v2.51] 개정이력: 사이드바·탭 클릭 시 문서 목록으로 이동 + 안내 */
...
/* ══════════════════════════════════════════════════
   D7: 연관 문서 추천 [v2.51 Phase 3]
   동일 태그 기반 연관 문서 패널 + 유사 문서 추천
   ══════════════════════════════════════════════════ */

/**
 * [v2.51] D7: 연관 문서 추천 페이지
 * [UI 구성]
 *  ① 문서 선택 → 해당 문서의 태그 표시
        ...
        <tbody>${activeUsers.length===0
          ...
          :activeUsers.map(u=>{
            ...
            return '<tr>'
              ...
              /* [v2.51 버그수정] E-MAIL td 누락 — 헤더 순서와 불일치로 최근로그인/비번 컬럼 밀림 */
  ...
  <div class="stab-pane" data-tab="general" style="display:block">
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="ts"><table class="dt" style="font-size:12px">
        ...
        <tbody>${(()=>{
          /* [v2.51] 최신순 정렬: created_at 없으면 date 기준 */
          ...
          return sorted.map((n,i)=>{
            ...
            return '<tr style="'+rowBg+'">'  /* [v2.51] 게시중 행 음영 */
...
async _renderSbDash(){
  ...
  /* ── 테이블 행 수 조회 ── */
  /* [v2.51] 테이블명 수정: documents → doc_master (v2.395 이후 변경됨) */
...
/* ══════════════════════════════════════════════════════════════
   제조설비관리 (EMS — Equipment Management System) [v2.51]
   ══════════════════════════════════════════════════════════════
   M1. eq_mgmt        설비 등록 관리
...
_eqSave:async function(editId){
  ...
  /* [v2.51] 설비번호: 입력값 우선, 없으면 DB에서 자동생성 */
  var inputNo = document.getElementById('efNo')?.value?.trim();
  /* [v2.51] 관리대장 기준 전체 필드 저장 */
  var row={
    name:name,
...
/* ──────────────────────────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────────- */
async eq_pm(){
  /* [v2.51] PM 화면 — 설비 목록 기반 (PM 로그 없어도 설비 표시) */
  var w=document.getElementById('pw');
...
async eq_as(){
  ...
  /* [v2.51] 반복고장 감지 — 동일 설비 + 동일 유형 3회 이상 */
  var repeatMap={};
  rows.forEach(function(r){
...
_asDetail:function(row){
  /* [v2.51] AS 상세보기 (읽기전용) → 수정 버튼으로 _asForm 진입 */
  var eqs=window._asEqs||[];
  var eq=(eqs||[]).find(function(e){return e.id===row.eq_id;})||{};
  var stCls={접수:'bred',처리중:'bblu',부품대기:'bamb',완료:'bgrn'};
...
_cardPrint:function(eqId){
  /* [v2.51] MY MACHINE CARD — 설비명세표 양식 완전 동일 재현 */
...
var SearchPop={
  ...
  _cfg:{
    ...
    /* [v2.51] EMS 설비 검색 */
    /* [v2.51 fix] ems_eq — SearchPop 표준 형식 (get 함수 포함) */
    ems_eq:{title:'설비 검색',
      fields:[
...
/* [v2.51] F3 근본 해결 — EMS 페이지명을 ems_eq 설정으로 직접 별칭 등록
