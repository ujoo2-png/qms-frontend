/* qms-pages.js — Pages 페이지 렌더러 [v2.318] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.318</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.318 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.318] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
async equip(){
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.318] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.318 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.318 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.318 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.318] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.318] 목록 헤더 */
    /* [v2.318] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.318 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.318 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.318 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.318] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.318 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
settings(){
  ...
  /* [v2.23] 3-A: 탭 구조 (기존 설정 + 사용자 관리)
     사용자 관리 탭은 관리자만 활성화 */
  /* [v2.318] renderTab — sbdash 전용 처리 포함 */
  const renderTab=(tab)=>{
  ...
  <div style="display:flex;gap:6px;margin-bottom:16px">
    ...
    <!-- [v2.318] SB 대시보드 탭 -->
    <!-- [v2.318] SB 대시보드 탭 - renderTab 후 패널 확실히 표시된 뒤 렌더 -->
...
/* [v2.318] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
  /* 최신 데이터 로드 */
...
/* [v2.318] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.318] SB 대시보드 — 파이차트 + 테이블 + keepalive */
  const el=document.getElementById('sbDashContainer');
  /* [v2.318] el 없으면 SB 탭이 열린 상태인지 확인 후 재시도 */
  if(!el){
    const pane=document.querySelector('.stab-pane[data-tab="sbdash"]');
    if(pane) pane.style.display='block';
    const el2=document.getElementById('sbDashContainer');
    if(!el2){Toast.show('SB 대시보드 패널을 찾을 수 없습니다.','warn');return;}
    return Pages._renderSbDash();
  }
  ...
  const DB_MAX=500, DB_USED=Math.max(0.1, Math.round(estBytes/(1024*1024)*10)/10);
  const DB_PCT=Math.min(Math.round((DB_USED/DB_MAX)*100),100);

  const ST_MAX=1024, ST_USED=0.5;
  const ST_PCT=Math.round((ST_USED/ST_MAX)*100)||1;

  /* 비활성 방지 D-day */
  const lastPing=localStorage.getItem('qms_keepalive')||'-';
  const daysLeft=lastPing==='-'?7:Math.max(0,7-Math.floor((new Date()-new Date(lastPing))/(864e5)));
    ...
    return `<div style="display:inline-flex;flex-direction:column;align-items:center;gap:6px">
      <svg width="120" height="120" viewBox="0 0 120 120">
        ...
        <text x="${CX}" y="${CY-6}" text-anchor="middle"
          font-size="14" font-weight="700" fill="#1e293b">${val}</text>
        <text x="${CX}" y="${CY+12}" text-anchor="middle"
          font-size="10" fill="#64748b">${sub}</text>
        <!-- 퍼센트 -->
        <text x="${CX}" y="${CY+26}" text-anchor="middle"
          font-size="10" fill="${color}" font-weight="600">${pct}%</text>
      </svg>
    </div>`;
  }
  ...
  el.innerHTML=
    '<div class="card" style="margin-bottom:14px">'+
    '<div class="ch">'+
    '<div class="ct">🔌 Supabase 현황</div>'+
    '<span style="font-size:11px;font-weight:600;color:'+(typeof _sb!=='undefined'&&_sb?'#22c55e':'#ef4444')+'">'+
      (typeof _sb!=='undefined'&&_sb?'● 연결됨':'● 미연결')+'</span>'+
    '</div>'+
    /* 파이차트 3개 */
    '<div style="display:flex;justify-content:space-around;align-items:flex-end;padding:16px 0">'+
    ...
    '<div style="text-align:center">'+
      ...
      donut(ST_PCT, ST_PCT>80?'#ef4444':ST_PCT>60?'#f59e0b':'#10b981', ST_USED+'MB', '/'+ST_MAX+'MB')+
    '</div>'+
    '<div style="text-align:center">'+
    ...
    '<th style="padding:6px 10px;min-width:120px">비율</th>'+
    '</tr></thead><tbody>'+
    tables.map(t=>{
      ...
      return '<tr style="border-bottom:1px solid var(--bd)">'+
        ...
        '<td style="padding:6px 10px">'+
          ...
          '<div style="background:#3b82f6;height:6px;width:'+pct2+'%;border-radius:4px;transition:.3s"></div>'+
          '</div>'+
        '</td>'+
        '</tr>';
    }).join('')+
    ...
    'Supabase 무료 플랜은 <strong>7일 미접속 시 일시정지</strong>됩니다.<br>'+
    ...
    '<span style="font-size:11px;color:'+(daysLeft<=2?'#ef4444':daysLeft<=4?'#f59e0b':'#22c55e')+'">'+
    (daysLeft<=2?'⚠️ 곧 비활성 위험!':daysLeft<=4?'주의 필요':'✅ 정상')+'</span>'+
    '</div>'+
    '</div>';
},

async _sbKeepAlive(){
  /* [v2.318] Supabase keepalive — 7일 비활성 방지 */
  try{
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.318] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.318] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
