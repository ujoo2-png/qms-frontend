/* qms-pages.js — Pages 페이지 렌더러 [v2.347] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.347</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.347 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.347] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
/* ── 사용자 ── */
async users(){
  /* [v2.347] 사원관리 — 인사 목록 (권한관리 없음, 활성사용자만) */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.347] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.347] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.347] status null인 데이터 실시간 재계산 */
  /* [v2.347] status 항상 재계산 — DB 저장값 무시하고 next 날짜 기준으로 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.347] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.347 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.347 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.347 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.347] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.347] 목록 헤더 */
    /* [v2.347] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.347 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.347 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.347 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.347] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.347 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
/* ── 설정 ── */
async settings(){
  /* [v2.347] settings 완전 재작성 — 3탭: 일반설정/사용자관리/SB대시보드 */
  const w=document.getElementById('pw');
  ...
  /* ── renderTab ── */
  /* [v2.347] renderTab — settings 내부만, 각 패널 명시적 제어 */
  const renderTab=(tab)=>{
  ...
  const renderUserMgmt=()=>{
    /* [v2.347] 사용자 등록 — admin 전용, 권한관리 포함 */
    const us=DB.users||[];
    ...
    const RC={admin:'background:#7c3aed;color:#fff',manager:'background:#2563eb;color:#fff',
      user:'background:#059669;color:#fff',viewer:'background:#64748b;color:#fff'};
    let h='';

    /* 승인 대기 */
    if(pnd.length){
      h+='<div class="card" style="margin-bottom:14px;border:1.5px solid #f59e0b">';
      h+='<div class="ch" style="background:#fef3c7"><div class="ct">⏳ 회원가입 승인 대기 ('+pnd.length+'명)</div></div>';
      h+='<table class="dt"><thead><tr><th>아이디</th><th>이름</th><th>부서</th><th>이메일</th><th>신청일</th><th style="text-align:center">처리</th></tr></thead><tbody>';
      pnd.forEach(u=>{
        h+='<tr>'
          ...
          +'<td>'+H.e(u.email||'-')+'</td>'
          +'<td style="font-size:11px">'+H.e((u.created_at||'').slice(0,10))+'</td>'
          +'<td style="text-align:center;white-space:nowrap">'
          +'<button class="btn bsm bgrn" onclick="Pages._approveUser('+u.id+')">✅ 승인</button> '
          +'<button class="btn bsm berr" onclick="Pages._rejectUser('+u.id+')">❌ 거절</button>'
          +'</td></tr>';
      });
      h+='</tbody></table></div>';
    }
    ...
    /* 사용자 목록 */
    h+='<div class="card"><div class="ch"><div class="ct">👤 사용자 목록 ('+act.length+'명)</div>'
      +'<button class="btn bpri bsm" onclick="Pages._uForm(null)">+ 사용자 등록</button></div>'
      +'<div class="ts"><table class="dt" style="font-size:12px"><thead><tr>'
      +'<th style="width:28px"><input type="checkbox" id="umgmtAllChk" onchange="document.querySelectorAll(&quot;.umgmt-chk&quot;).forEach(c=>c.checked=this.checked)"></th>'
      ...
      +'<th style="width:60px">상태</th>'
      +'<th style="width:86px">등록일</th>'
      +'<th style="width:86px">수정일</th>'
    ...
    if(act.length===0){
      h+='<tr><td colspan="14" style="text-align:center;padding:20px;color:var(--tm)">등록된 사용자가 없습니다.</td></tr>';
    } else {
      act.forEach((u,i)=>{
        const uid=String(u.id);
        const roleOpts=ROLES.map(r=>'<option value="'+r+'"'+(u.role===r?' selected':'')+'>'+RL[r]+'</option>').join('');
        h+='<tr>'
          +'<td><input type="checkbox" class="umgmt-chk" value="'+uid+'"></td>'
          +'<td style="text-align:center;color:var(--tm)">'+(i+1)+'</td>'
          +'<td style="color:var(--tm);font-size:11px">'+H.e(u.username)+'</td>'
          +'<td><strong style="cursor:pointer;color:var(--pri)" onclick="Pages._uFormById('+uid+')">'+H.e(u.name||u.username)+'</strong></td>'
          +'<td>'+H.e(u.department||'-')+'</td>'
          ...
          +'<td>'
          +'<span class="badge" style="'+RC[u.role||'user']+';font-size:10px">'+RL[u.role||'user']+'</span>'
          +'<select class="fsel" style="font-size:10px;margin-top:2px" onchange="Pages._setUserRole('+uid+',this.value)">'+roleOpts+'</select>'
          +'</td>'
          +'<td><span class="badge '+(u.active?'bgrn':'bgry')+'" style="cursor:pointer" onclick="Pages._uStatusPopup('+uid+')">'+(u.active?'활성':'비활성')+'</span></td>'
          +'<td style="font-size:11px;color:var(--tm)">'+H.e((u.created_at||'').slice(0,10)||'-')+'</td>'
          +'<td style="font-size:11px;color:var(--tm)">'+H.e((u.updated_at||'').slice(0,10)||'-')+'</td>'
          +'<td style="font-size:11px;color:var(--tm)">'+H.e((u.last_login||'').slice(0,10)||'-')+'</td>'
          +'<td style="text-align:center">'
          +'<button class="btn bxs bgh" onclick="Pages._uFormById('+uid+')">수정</button> '
          +'<button class="btn bxs berr" onclick="Pages._uDelete('+uid+')">삭제</button>'
          +'</td>'
          +'<td><button class="btn bsm bamb" onclick="Pages._uResetPw('+uid+',&quot;'+H.e(u.username)+'&quot;)">🔑</button></td>'
          +'</tr>';
      });
    }
...
/* [v2.347] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  /* [v2.347] 사용자 등록/권한 관리 — 완전 독립 구현 */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
  const fresh=await SB.getUsers();
  if(fresh) DB.users=fresh;
  Pages._renderSysUsers(w);
},

_renderSysUsers(w=null){
  /* [v2.347] 사용자 등록/권한 관리 독립 렌더 */
  /* [v2.347] w가 있으면 레이아웃 생성 */
  if(w){
...
/* [v2.347] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.347] SB 대시보드 — 완전 재작성 */
...
async _sbKeepAlive(){
  /* [v2.347] Supabase keepalive — 7일 비활성 방지 */
  try{
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.347] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.347] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
