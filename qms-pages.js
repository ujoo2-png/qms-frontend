/* qms-pages.js — Pages 페이지 렌더러 [v2.343] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.343</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.343 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.343] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.343] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.343] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.343] status null인 데이터 실시간 재계산 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.343] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.343 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.343 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.343 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.343] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.343] 목록 헤더 */
    /* [v2.343] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.343 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.343 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.343 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.343] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.343 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
/* ── 설정 ── */
async settings(){
  /* [v2.343] settings 완전 재작성 — 3탭: 일반설정/사용자관리/SB대시보드 */
  const w=document.getElementById('pw');
  ...
  /* ── renderTab ── */
  /* [v2.343] renderTab — settings 내부만, 각 패널 명시적 제어 */
  const renderTab=(tab)=>{
...
/* [v2.343] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  /* [v2.343] 사용자 등록/권한 관리 — 완전 독립 구현 */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
  const fresh=await SB.getUsers();
...
_renderSysUsers(w=null){
  /* [v2.343] 사용자 등록/권한 관리 독립 렌더 */
  /* [v2.343] w가 있으면 레이아웃 생성 */
  if(w){
...
/* [v2.343] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.343] SB 대시보드 — 전체 테이블 누락 없이 */
  /* [v2.343] sbDashContainer — sbdash 패널 직접 표시 */
  const _pw=document.getElementById('pw');
  if(_pw){
    /* sbdash 패널만 표시, general/usermgmt 숨김 */
    ['general','usermgmt','sbdash'].forEach(tab=>{
      const p=_pw.querySelector(`.stab-pane[data-tab="${tab}"]`);
      if(p) p.style.display=(tab==='sbdash')?'block':'none';
    });
    /* 탭 버튼 활성화 */
  ...
  el.innerHTML=
    ...
    /* [v2.343] 도넛 차트 5개 — DB/Storage/Egress/전체행/비활성방지 */
    const EG_MAX=5, EG_USED=0.1;  /* Egress: 실측 불가 → 보수적 추정 */
    const EG_PCT=Math.round((EG_USED/EG_MAX)*100)||1;
    +'<div style="display:flex;justify-content:space-around;flex-wrap:wrap;gap:8px;padding:6px 0">'
    +'<div style="text-align:center">'
      +'<div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">📊 Database</div>'
      +donut(DB_PCT,DB_PCT>80?'#ef4444':DB_PCT>50?'#f59e0b':'#3b82f6',DB_USED+'MB','/'+DB_MAX+'MB')+'</div>'
    +'<div style="text-align:center">'
      +'<div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">💾 Storage</div>'
      +donut(ST_PCT,'#10b981','0.5MB','/1GB')+'</div>'
    +'<div style="text-align:center">'
      +'<div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">🌐 Egress</div>'
      +donut(EG_PCT,'#f59e0b',EG_USED+'GB','/'+EG_MAX+'GB')+'</div>'
    +'<div style="text-align:center">'
      +'<div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">📋 전체 행</div>'
      +donut(Math.min(100,Math.round(totalRows/500)),'#8b5cf6',totalRows,'rows')+'</div>'
    +'<div style="text-align:center">'
      +'<div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">⏰ 비활성 방지</div>'
...
async _sbKeepAlive(){
  /* [v2.343] Supabase keepalive — 7일 비활성 방지 */
  try{
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.343] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.343] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
