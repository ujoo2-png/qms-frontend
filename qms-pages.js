/* qms-pages.js — Pages 페이지 렌더러 [v2.398.5]
...
async doc_approval(){
  ...
  try{
    /* [v2.398.5] users 테이블(설정→사용자관리)에서 현재 로그인 사용자 매칭
       Auth._u = users row 전체. id/name/username 순으로 매칭 */
    var users=await SB.getUsers();
    var meId=null;

    /* [v2.398.5] 사용자 매칭 강화 — 설정→사용자관리 users 테이블과 연동
...
/* ══════════════════════════════════════════════════
   D4: 개정 이력 타임라인 [v2.398.5]
   ══════════════════════════════════════════════════ */
/* [v2.398.5] 개정이력: 사이드바·탭 클릭 시 문서 목록으로 이동 + 안내 */
...
async doc_history(docId){
  ...
  try{
    ...
    /* [v2.398.5] 문서 정보 배너 — 깔끔한 카드 그리드 UI */
...
/* ══════════════════════════════════════════════════
   D7: 연관 문서 추천 [v2.398.5 Phase 3]
   동일 태그 기반 연관 문서 패널 + 유사 문서 추천
   ══════════════════════════════════════════════════ */

/**
 * [v2.398.5] D7: 연관 문서 추천 페이지
 * [UI 구성]
 *  ① 문서 선택 → 해당 문서의 태그 표시
...
/* ══════════════════════════════════════════════════
   D8: 문서 현황 대시보드 [v2.398.5 Phase 4]
   KPI 카드 · 유형 분포 · 상태 현황 · 심사 준비율 게이지
   ══════════════════════════════════════════════════ */

/**
 * [v2.398.5] D8: 문서 현황 대시보드
 * [UI 구성]
...
async settings(){
  ...
  const isAdmin=Auth._u?.role==='admin';
  /* [v2.398.5] 공지사항 최신 로드 — Supabase 영속화
        ...
        <tbody>${activeUsers.length===0
          ...
          :activeUsers.map(u=>{
            ...
            return '<tr>'
              ...
              /* [v2.398.5 버그수정] E-MAIL td 누락 — 헤더 순서와 불일치로 최근로그인/비번 컬럼 밀림 */
  ...
  <div class="stab-pane" data-tab="general" style="display:block">
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="ts"><table class="dt" style="font-size:12px">
        ...
        <tbody>${(()=>{
          /* [v2.398.5] 최신순 정렬: created_at 없으면 date 기준 */
          ...
          return sorted.map((n,i)=>{
            ...
            return '<tr style="'+rowBg+'">'  /* [v2.398.5] 게시중 행 음영 */
...
async _renderSbDash(){
  ...
  /* ── 테이블 행 수 조회 ── */
  /* [v2.398.5] 테이블명 수정: documents → doc_master (v2.395 이후 변경됨) */
