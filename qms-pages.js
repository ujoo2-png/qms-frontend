/* qms-pages.js — Pages 페이지 렌더러 [v2.330] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.330</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.330 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.330] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.330] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.330] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.330] status null인 데이터 실시간 재계산 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.330] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.330 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.330 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.330 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.330] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.330] 목록 헤더 */
    /* [v2.330] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.330 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.330 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.330 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.330] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.330 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
settings(){
  ...
  /* [v2.23] 3-A: 탭 구조 (기존 설정 + 사용자 관리)
     사용자 관리 탭은 관리자만 활성화 */
  /* [v2.330] renderTab — sbdash 전용 처리 포함 */
  const renderTab=(tab)=>{
    ...
    /* [v2.330] sbdash 탭: 항상 _renderSbDash 실행 */
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
    ...
    <!-- [v2.330] Supabase 대시보드 (별도 카드, 상하 여백 명확) -->
...
/* [v2.330] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.330] SB 대시보드 — 전체 테이블 누락 없이 */
  /* [v2.330] sbDashContainer — general 탭이 숨겨져도 getElementById는 작동 */
  ...
  el.innerHTML=
    ...
    /* [v2.330] 도넛 차트 5개 — DB/Storage/Egress/전체행/비활성방지 */
...
async _sbKeepAlive(){
  /* [v2.330] Supabase keepalive — 7일 비활성 방지 */
  try{
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.330] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.330] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
