/* qms-pages.js — Pages 페이지 렌더러 [v2.372] */
"use strict";
...
/* ── 홈 (메인화면) ──
   ...
   v2.372: C안 우측 패널 고정, 카드 높이 5배, stopPropagation 제거 */
home(){
  ...
  const cards=[
    ...
    {c:'mc-c4',icon:'⭐',name:'공급업체 품질',badge:0,
     subs:[{icon:'📊',label:'SQM 대시보드',page:'sqm_dash'},{icon:'⭐',label:'업체 평가',page:'sqm_eval'},{icon:'🔎',label:'업체 심사',page:'sqm_audit'},{icon:'📅',label:'심사 계획 관리',page:'sqm_plan'},{icon:'🚚',label:'납품 이력',page:'sqm_delivery'}]},
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.372</div>
        </div>
...
/* ── 개인정보 수정 ── */
/* ── 멘션 답장 [v2.372→v2.372] ── */
  /* [v2.372 A안] _mentionReply 전체 재작성 — 문자열 연결 방식 (백틱 중첩 제거) */
...
async _mentionReplySend(parentId){
  /* [v2.372 PhaseB] thread_id 기반 SB 스레드 저장 */
...
async _approveUser(userId, username){
  ...
  /* [v2.372] 승인 후 화면 유지 — usermgmt 탭 그대로 */
...
/* 접근 권한 저장 (sessionStorage, v2.372) */
/* [v2.372] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
async _changePw(){
  ...
  /* [v2.372] admin 비밀번호 변경 — sessionStorage 저장 */
...
/* [v2.372] 설정 사용자관리 이름 클릭 → id로 사용자 찾아 수정 팝업 */
_uFormById(userId){
...
/* ── 품목 상세/등록/수정 팝업 ──
   [v2.372] 행 클릭 또는 품목코드 클릭 시 상세 팝업 열림 */
_iForm(row=null){
  const e=!!row;
...
/* [v2.372 수정] async로 전환 + SB.addVendor/updateVendor 연동
...
/* ── 사용자 ── */
async users(){
  /* [v2.372] 사원관리 — 인사 목록 (권한관리 없음, 활성사용자만) */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
...
/* [v2.372] A2: SHA-256 해시 저장 + password 컬럼 포함
...
async _uSave(id){
  ...
  if(id){
    ...
    /* [v2.372] admin 계정 이메일 수정 시 로그인창 footer 반영 */
  ...
  /* [v2.372] 저장 후 사원관리 이동 제거 — 사용자관리 탭 유지 */
  Modal.close();
  /* [v2.372] settings 사용자관리 탭 내 목록 갱신 — window.renderUserMgmt 사용 */
...
/* ── 임시비밀번호 발급 (관리자 전용, B2안) ──
   [v2.372] 관리자가 사용자 비밀번호를 임시값으로 초기화
...
/* ══════════════════════════════════════════════════════
   ...
   - v2.372: 구매검사(insp_pu), 외주검사(insp_ou) 추가
...
async _insp(type){
  ...
  /* [v2.372 복원] _sbFetchAll 전체 로드 후 로컬 필터 방식
     Phase1은 DOM 순서 문제로 무한 스피너 버그 발생 → 복원 */
  w.innerHTML='<div class="spin"></div>';
  /* [v2.372] 항상 SB에서 최신 데이터 로드 */
...
_ncForm(row=null){
  /* [v2.372] 부적합 등록/수정 폼 — 사내외/품목코드/고객 유형 추가 */
  const isEdit=!!row;
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.372] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.372] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.372] status null인 데이터 실시간 재계산 */
  /* [v2.372] status 항상 재계산 — DB 저장값 무시하고 next 날짜 기준으로 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.372] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
async _eqSave(orig){
  ...
  /* [v2.372 Phase1] 수정 시 변경 이력 자동 기록 */
  if(orig){
...
_eqDetail(row){
  /* [v2.372 Phase2] 탭 구조: 기본정보 / 교정이력 / 변경이력 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.372 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
async _calSave(orig){
  /* [v2.372 Phase2] 교정 저장 + equipment.next 자동 갱신 */
  const g=id=>(document.getElementById(id)?.value||'').trim();
...
/* [v2.372 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.372 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
docs(){
  ...
  Tbl.render({el:'#docTbl',cols:[
    ...
    {key:'status',label:'상태',w:'62px',render:v=>`<span class="badge ${v==='유효'?'bgrn':v==='초안'?'bamb':'bgry'}">${H.e(v)}</span>`},
      ...
      Modal.confirm({
        title:'🗑️ 문서관리 삭제 확인',
        msg:'<div style="text-align:center"><div style="font-size:28px">⚠️</div>'+`<div style="font-size:14px;font-weight:700;margin:6px 0">선택한 <b style="color:#dc2626">${ids.length}건</b>의 문서관리를 삭제합니다.</div>`+'<div style="font-size:12px;color:#64748b">삭제된 데이터는 복구가 어렵습니다. 계속하시겠습니까?</div></div>',
        danger:true,
        onOk:_doDelete
      });
...
_mentionBuildLayout(w,me,isAdmin){
  const users=(DB.users||[]).filter(u=>u.active!==0&&!u.pending);
  const userOpts=users.map(u=>`<option value="${H.e(u.username)}">${H.e(u.name||u.username)} (${H.e(u.department||'')})</option>`).join('');

  w.innerHTML=`
    <div id="mentionWrap">
    <!-- KPI -->
    <div class="stat-dash" id="mentionKpi"></div>

    <!-- 페이지 헤더 -->
    <div class="ph" style="margin-top:14px">
      <div><div class="ptit">💬 멘션함</div>
        <div class="psub">팀원 알림 · 업무 협업 · 부적합/교정 자동 멘션</div></div>
      <div class="pac">
        <button class="btn bpri" onclick="Pages._mentionForm()">✏️ 새 멘션 작성</button>
      </div>
    </div>

    <!-- 탭 -->
    <div class="stabs" style="margin-bottom:10px">
      <button class="stab-btn on" data-tab="inbox"  onclick="Pages._mentionTab('inbox',this)">📥 받은함</button>
      <button class="stab-btn"    data-tab="sent"   onclick="Pages._mentionTab('sent',this)">📤 보낸함</button>
      <button class="stab-btn"    data-tab="all"    onclick="Pages._mentionTab('all',this)">${isAdmin?'📋 전체':'📋 전체'}</button>
    </div>
    ...
    <div class="tbar">
      <div class="sw2">
        <input type="text" id="mentionSearch" placeholder="내용, 발신자, 수신자 검색..."
          oninput="Pages._mentionRefresh()">
      </div>
      <select class="fsel" id="mentionReadF" onchange="Pages._mentionRefresh()">
        <option value="">전체</option>
        <option value="unread">읽지 않음</option>
        <option value="read">읽음</option>
      </select>
      <button class="btn bout bsm" onclick="Pages._mentionMarkAllRead()">✅ 전체 읽음</button>
    </div>

    <!-- 목록 -->
    <div id="mentionList"></div>

    <!-- 새 멘션 작성 모달용 숨김 select -->
...
_mentionRefresh(){
  ...
  let filtered=[];
  if(tab==='inbox'){
    filtered=all.filter(m=>m.to===me||(m.to_list||[]).includes(me)||
      (isAdmin&&m.to==='all'));
  } else if(tab==='sent'){
    filtered=all.filter(m=>m.from===me);
  } else {
    filtered=isAdmin?all:all.filter(m=>m.to===me||m.from===me||(m.to_list||[]).includes(me));
  }
  ...
  const unread=all.filter(m=>(m.to===me||(m.to_list||[]).includes(me))&&!m.read).length;
  const sent=all.filter(m=>m.from===me).length;
  ...
  if(kpiEl) kpiEl.innerHTML=`
    <div class="sd-card"><div class="sd-icon" style="background:#eff6ff;color:#2563eb">💬</div>
      <div><div class="sd-val">${total}</div><div class="sd-lbl">전체</div></div></div>
    <div class="sd-card"><div class="sd-icon" style="background:#fee2e2;color:#dc2626">🔴</div>
      <div><div class="sd-val">${unread}</div><div class="sd-lbl">읽지않음</div></div></div>
    <div class="sd-card"><div class="sd-icon" style="background:#f0fdf4;color:#059669">📤</div>
      <div><div class="sd-val">${sent}</div><div class="sd-lbl">보낸함</div></div></div>
    <div class="sd-card"><div class="sd-icon" style="background:#fef3c7;color:#d97706">📋</div>
      <div><div class="sd-val">${filtered.length}</div><div class="sd-lbl">현재 필터</div></div></div>`;

  const listEl=document.getElementById('mentionList');
  if(!listEl) return;

  if(!filtered.length){
    listEl.innerHTML='<div class="card" style="text-align:center;padding:40px;color:var(--tl)"><div style="font-size:32px;margin-bottom:8px">💬</div><div style="font-size:13px">멘션이 없습니다.</div></div>';
    return;
  }
  ...
  filtered.forEach(m=>{
    ...
    const refBadge=m.ref?`<span class="badge bgry" style="font-size:10px;margin-left:4px">${H.e(m.ref)}</span>`:'';
    const replyBadge=m.reply_to?'<span class="badge bblu" style="font-size:10px;margin-left:4px">↩답장</span>':'';
    const unreadDot=isUnread?'<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#dc2626;margin-right:6px;vertical-align:middle"></span>':'';

    h+=`<div class="card" style="padding:14px 18px;margin-bottom:8px;${isUnread?'border-left:3px solid #dc2626;':'border-left:3px solid #e2e8f0;'}cursor:pointer"
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
        ...
        ${replyBadge}${refBadge}
        <span style="font-size:11px;color:var(--tm);margin-left:auto">${ts}</span>
        <button class="btn bxs berr" style="font-size:10px;padding:1px 6px"
          onclick="event.stopPropagation();Pages._mentionDel(${m.id})">🗑</button>
      </div>
      <div style="font-size:13px;color:${isUnread?'#1e293b':'#64748b'};font-weight:${isUnread?'600':'400'};
        white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
        ${H.e(txt.slice(0,100))}</div>
      <div style="display:flex;gap:6px;margin-top:8px">
        <button class="btn bxs bout" style="font-size:11px;padding:2px 10px"
          onclick="event.stopPropagation();Pages._mentionReply(${m.id},'${H.e(m.from||'')}')">↩ 답장</button>
        ${!m.read&&m.from!==me?`<button class="btn bxs" style="font-size:11px;padding:2px 10px;background:#e0f2fe;color:#0891b2"
          onclick="event.stopPropagation();Pages._mentionMarkRead(${m.id})">✅ 읽음</button>`:''}
      </div>
    </div>`;
  });
  listEl.innerHTML=h;
...
async _mentionDetail(id){
  ...
  Modal.open({
    ...
    size:'mmd',
    foot:`<button class="btn bout" onclick="Modal.close()">닫기</button>`
        +`<button class="btn bgry bsm" onclick="Modal.close();Pages._mentionReply(${m.id},'${H.e(m.from||'')}')">↩ 답장</button>`,
    body:`<div class="card" style="padding:14px 18px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0">
        <div class="ir"><div class="il">발신자</div><div class="iv" style="font-weight:700">${H.e(m.from||'-')}</div></div>
        <div class="ir"><div class="il">수신자</div><div class="iv">${H.e(m.to||'-')}</div></div>
        <div class="ir"><div class="il">부서</div><div class="iv">${H.e(m.dept||'-')}</div></div>
        <div class="ir"><div class="il">시간</div><div class="iv" style="color:var(--tm)">${ts}</div></div>
...
async _mentionRead(id){
  ...
  await SB.updateMention(id,{read:true});
  m.read=true;
  /* [v2.372 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.372 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.372 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.372] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.372 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
/* ── 설정 ── */
async settings(){
  /* [v2.372] settings 완전 재작성 — 3탭: 일반설정/사용자관리/SB대시보드 */
  const w=document.getElementById('pw');
  ...
  /* ── renderTab ── */
  /* [v2.372] renderTab — settings 내부만, 각 패널 명시적 제어 */
  const renderTab=(tab)=>{
    const pw=document.getElementById('pw');
    if(!pw) return;
    /* 탭 버튼 활성화 */
    pw.querySelectorAll('.stab-btn').forEach(b=>{
      b.classList.toggle('on', b.dataset.tab===tab);
    });
    /* 패널 명시적 hide/show */
    const panes=['general','usermgmt','sbdash'];
    panes.forEach(p=>{
      const el=pw.querySelector(`.stab-pane[data-tab="${p}"]`);
  ...
  /* ── renderUserMgmt ── */
  const renderUserMgmt=(sortKey='name',sortDir=1)=>{
    /* [v2.372] 사용자 관리 — 정렬 기능 + 12px + 30px 행높이 */
    const us=DB.users||[];
    const pnd=us.filter(u=>u.pending);
    const act=us.filter(u=>!u.pending&&u.active!==0);
    const RL={admin:'관리자',manager:'매니저',user:'사용자',viewer:'뷰어'};
    const RC={admin:'#7c3aed',manager:'#2563eb',user:'#059669',viewer:'#64748b'};
    ...
    /* 스타일 상수 */
    const TH=(align='center')=>'padding:5px 8px;font-size:12px;font-weight:700;color:#fff;background:#1e293b;'
      +'border:1px solid #334155;white-space:nowrap;text-align:'+align+';cursor:pointer;user-select:none';
    const TD=(align='left')=>'padding:4px 8px;font-size:12px;color:#374151;border-bottom:1px solid #e2e8f0;'
      +'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;vertical-align:middle;text-align:'+align;
    ...
    /* ── 승인 대기 ── */
    if(pnd.length){
      h+='<div style="margin-bottom:12px;border:1.5px solid #f59e0b;border-radius:8px;overflow:hidden">'
        +'<div style="background:#fef3c7;padding:8px 14px;display:flex;align-items:center;gap:8px">'
        +'<span style="font-size:13px;font-weight:700;color:#92400e">⏳ 회원가입 승인 대기</span>'
        +'<span style="background:#f59e0b;color:#fff;font-size:11px;font-weight:700;padding:2px 10px;border-radius:20px">'+pnd.length+'명</span>'
        +'</div>'
        +'<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;table-layout:fixed">'
        +'<colgroup><col style="width:14%"><col style="width:11%"><col style="width:12%">'
        +'<col style="width:20%"><col style="width:10%"><col style="width:33%"></colgroup>'
        +'<thead><tr>'
        +'<th style="'+TH()+'">아이디</th><th style="'+TH()+'">이름</th><th style="'+TH()+'">부서</th>'
        +'<th style="'+TH()+'">이메일</th><th style="'+TH()+'">신청일</th><th style="'+TH()+'">처리</th>'
        +'</tr></thead><tbody>';
      pnd.forEach((u,i)=>{
        h+='<tr style="height:30px;background:'+(i%2===0?'#fff':'#f8fafc')+'">'
          +'<td style="'+TD()+'"><strong>'+H.e(u.username)+'</strong></td>'
          +'<td style="'+TD()+'">'+H.e(u.name)+'</td>'
          +'<td style="'+TD()+'">'+H.e(u.department||'-')+'</td>'
          +'<td style="'+TD()+'">'+H.e(u.email||'-')+'</td>'
          +'<td style="'+TD()+'">'+H.e((u.created_at||'').slice(0,10))+'</td>'
          +'<td style="'+TD('center')+';">'
          +'<button class="btn bsm bgrn" style="font-size:11px;padding:2px 10px;height:22px" data-uid="'+u.id+'" onclick="Pages._approveUser(+this.dataset.uid)">✅ 승인</button> '
          +'<button class="btn bsm berr" style="font-size:11px;padding:2px 10px;height:22px" data-uid="'+u.id+'" onclick="Pages._rejectUser(+this.dataset.uid)">❌ 거부</button>'
          +'</td></tr>';
      });
    ...
    h+='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">'
      +'<div style="font-size:14px;font-weight:700;color:#1e293b">👤 사용자 목록'
      +'<span style="margin-left:8px;background:#2563eb;color:#fff;font-size:11px;padding:2px 10px;border-radius:12px">'+act.length+'명</span>'
      +'</div>'
      +'<button class="btn bpri" style="font-size:12px;font-weight:600;padding:5px 14px;height:30px" onclick="Pages._uForm(null)">+ 사용자 등록</button>'
      +'</div>';

    /* ── 사용자 목록 테이블 ── */
    /* 클릭 정렬 헬퍼 */
    const SH=(key,lbl,w='')=>'<th style="'+TH()+(w?';width:'+w:'')+'" onclick="window._umSort(\''+key+'\')">'
      +lbl+sortArrow(key)+'</th>';

    h+='<div style="overflow-x:auto;border-radius:6px;border:1px solid #e2e8f0">'
      +'<table style="width:100%;border-collapse:collapse;table-layout:auto;font-size:12px">'
      +'<thead><tr style="height:32px">'
      +'<th style="'+TH()+';width:28px"><input type="checkbox" id="umgmtAllChk" onchange="document.querySelectorAll(&quot;.umgmt-chk&quot;).forEach(c=>c.checked=this.checked)"></th>'
      +SH('_idx','No','32px')
      +SH('name','이름')
      +SH('username','아이디')
      +SH('department','부서')
      +SH('tel','연락처','100px')
      +SH('email','E-MAIL','140px')
      +'<th style="'+TH()+';width:100px">권한</th>'
      +SH('active','상태','58px')
      +SH('created_at','등록일','80px')
      +SH('updated_at','수정일','80px')
      +SH('last_login','최근로그인','90px')
      +'<th style="'+TH()+';width:88px">관리</th>'
      +'<th style="'+TH()+';width:46px">초기화</th>'
      +'</tr></thead><tbody>';

    if(sorted.length===0){
      h+='<tr><td colspan="14" style="text-align:center;padding:24px;font-size:12px;color:#94a3b8">등록된 사용자가 없습니다.</td></tr>';
    } else {
      sorted.forEach((u,i)=>{
        const uid=String(u.id);
        const roleOpts=ROLES.map(r=>'<option value="'+r+'"'+(u.role===r?' selected':'')+'>'+RL[r]+'</option>').join('');
        h+='<tr style="height:30px;background:'+(i%2===0?'#fff':'#f8fafc')+';transition:background .1s" '
          +'onmouseover="this.style.background=\'#eff6ff\'" onmouseout="this.style.background=\''+(i%2===0?'#fff':'#f8fafc')+'\'">'
          +'<td style="'+TD('center')+';width:28px"><input type="checkbox" class="umgmt-chk" value="'+uid+'"></td>'
          +'<td style="'+TD('center')+';color:#94a3b8;width:32px">'+(i+1)+'</td>'
          +'<td style="'+TD()+'">'
            +'<a style="color:#2563eb;font-weight:600;cursor:pointer;text-decoration:none" onclick="Pages._uFormById('+uid+')">'+H.e(u.name||u.username)+'</a>'
          +'</td>'
          +'<td style="'+TD()+';color:#64748b">'+H.e(u.username)+'</td>'
          +'<td style="'+TD()+'">'+H.e(u.department||'-')+'</td>'
          +'<td style="'+TD()+';width:100px">'+H.e(u.tel||u.phone||'-')+'</td>'
          +'<td style="'+TD()+';width:140px">'+(u.email?'<a href="mailto:'+H.e(u.email)+'" style="color:#2563eb;text-decoration:none">'+H.e(u.email)+'</a>':'-')+'</td>'
          +'<td style="'+TD()+';width:100px">'
            +'<span style="background:'+RC[u.role||'user']+';color:#fff;font-size:10px;font-weight:700;padding:1px 7px;border-radius:10px">'+RL[u.role||'user']+'</span>'
            +'<br><select style="font-size:10px;width:100%;margin-top:2px;border:1px solid #e2e8f0;border-radius:3px;height:18px" data-uid="'+uid+'" onchange="Pages._setUserRole(+this.dataset.uid,this.value)">'+roleOpts+'</select>'
          +'</td>'
          +'<td style="'+TD('center')+';width:58px">'
            +'<span style="background:'+(u.active?'#059669':'#94a3b8')+';color:#fff;font-size:10px;font-weight:700;padding:2px 6px;border-radius:10px;cursor:pointer" data-uid="'+uid+'" onclick="Pages._uStatusPopup(+this.dataset.uid)">'+(u.active?'활성':'비활성')+'</span>'
          +'</td>'
          +'<td style="'+TD()+';color:#94a3b8;width:80px">'+H.e((u.created_at||'').slice(0,10)||'-')+'</td>'
          +'<td style="'+TD()+';color:#94a3b8;width:80px">'+H.e((u.updated_at||'').slice(0,10)||'-')+'</td>'
          +'<td style="'+TD()+';color:#94a3b8;width:90px">'+H.e((u.last_login||'').slice(0,10)||'-')+'</td>'
          +'<td style="'+TD('center')+';width:88px;white-space:nowrap">'
            +'<button style="font-size:11px;padding:2px 8px;height:22px;background:#fff;color:#374151;border:1px solid #d1d5db;border-radius:4px;cursor:pointer" data-uid="'+uid+'" onclick="Pages._uFormById(+this.dataset.uid)">수정</button> '
            +'<button style="font-size:11px;padding:2px 8px;height:22px;background:#fee2e2;color:#dc2626;border:1px solid #fecaca;border-radius:4px;cursor:pointer" data-uid="'+uid+'" onclick="Pages._uDelete(+this.dataset.uid)">삭제</button>'
          +'</td>'
          +'<td style="'+TD('center')+';width:46px">'
            +'<button style="font-size:11px;padding:2px 6px;height:22px;background:#ede9fe;color:#7c3aed;border:1px solid #ddd6fe;border-radius:4px;cursor:pointer" data-uid="'+uid+'" data-uname="'+H.e(u.username)+'" onclick="Pages._uResetPw(+this.dataset.uid,this.dataset.uname)">🔑</button>'
          +'</td>'
          +'</tr>';
      });
    }
    ...
    window._umSort=(key)=>{
      ...
      if(!uPane) return;
      /* 현재 정렬 상태 유지 */
      const cur=window._umSortState||{key:'name',dir:1};
      const newDir=cur.key===key?-cur.dir:1;
      window._umSortState={key,dir:newDir};
      if(typeof window.renderUserMgmt==='function'){
        uPane.innerHTML=window.renderUserMgmt(key,newDir)+(typeof window.renderPermTable==='function'?window.renderPermTable():'');
      }
    };
    window._umSortState=window._umSortState||{key:sortKey,dir:sortDir};
    return h;
  };
    ...
    return `<div class="card">
      ...
      <div class="ts"><table class="dt" style="font-size:12px">
        ...
        <tbody>${MENU_GROUPS.map(g=>`
          <tr><td colspan="5" style="background:var(--bg2);font-weight:700;padding:6px 10px;font-size:11px">${g.group}</td></tr>
          ${g.pages.map(p=>`<tr>
  ...
  <div style="display:flex;gap:6px;margin-bottom:16px">
    ...
    <button class="btn stab-btn bout" data-tab="usermgmt"
      onclick="renderTab('usermgmt')"
      style="border-radius:8px">👥 사용자 관리</button>
  ...
  <div class="stab-pane" data-tab="general" style="display:block">
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="ts"><table class="dt" style="font-size:12px">
        <thead><tr>
          ...
          <th style="width:92px">시작일</th><th style="width:92px">종료일</th>
          <th style="width:72px;text-align:center">게시</th>
        ...
        <tbody>${notices.length===0
          ...
          :notices.map((n,i)=>{
            ...
            return '<tr>'
              ...
              +'<td style="text-align:center">'+(n.file?'<span>📎</span>':'-')+'</td>'
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="card">
        ...
        <div id="logoPreview" style="height:48px;display:flex;align-items:center;justify-content:center;margin-bottom:8px">
...
`;
  ...
  /* [v2.372] renderUserMgmt, renderPermTable을 window에 노출 → _uSave에서 접근 가능 */
  window.renderTab=renderTab;
  window.renderUserMgmt=renderUserMgmt;
  window.renderPermTable=renderPermTable;
},
/* [v2.372] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  /* [v2.372] 사용자 등록 → settings() 로드 후 usermgmt 탭 즉시 활성화 */
  await Pages.settings();
  /* settings() 렌더 완료 후 usermgmt 탭 강제 전환 */
  const pw=document.getElementById('pw');
  if(pw){
    pw.querySelectorAll('.stab-btn').forEach(b=>b.classList.toggle('on',b.dataset.tab==='usermgmt'));
    pw.querySelectorAll('.stab-pane').forEach(p=>{
      p.style.display=p.dataset.tab==='usermgmt'?'block':'none';
    });
    /* usermgmt 패널 내용 직접 렌더 */
    const uPane=pw.querySelector('.stab-pane[data-tab="usermgmt"]');
    if(uPane&&uPane.innerHTML.trim()===''){
      uPane.innerHTML=renderUserMgmt();
    }
  }
},

_renderSysUsers(w=null){
  /* [v2.372] 사용자 등록/권한 관리 독립 렌더 */
  /* [v2.372] w가 있으면 레이아웃 생성 */
  if(w){
    w.innerHTML='<div class="ph"><div><div class="ptit">👤 사용자 등록 / 권한 관리</div></div><div class="pac"><button class="btn bpri btn-f2" onclick="Pages._addUser()">+ 사용자 등록</button></div></div><div id="sysUserBody"></div>';
  }
  ...
  el.innerHTML=
    (pending.length?
      ...
      +pending.map(u=>'<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--bg2);border-radius:6px">'
        ...
        +'<button class="btn bsm bgrn" data-uid="'+u.id+'"'
         +' onclick="Pages._approveUser(+this.dataset.uid)">✅ 승인</button>'
        +'<button class="btn bsm berr" data-uid="'+u.id+'" style="margin-left:4px"'
         +' onclick="Pages._rejectUser(+this.dataset.uid)">❌ 거부</button>'
        +'</div></div>').join('')
      +'</div></div>':'')
    +'<div class="card">'
    ...
    +active.map(u=>'<tr style="border-bottom:1px solid var(--bd)">'
      ...
      +'<select class="fsel" style="font-size:10px;padding:2px 4px" onchange="Pages._setUserRole('+u.id+',this.value)">'
...
/* [v2.372] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.372] SB 대시보드 — 완전 재작성 */
  const _pw=document.getElementById('pw');
  if(_pw){
    ['general','usermgmt','sbdash'].forEach(t=>{
      const p=_pw.querySelector(`.stab-pane[data-tab="${t}"]`);
      if(p) p.style.display=(t==='sbdash')?'block':'none';
    });
    _pw.querySelectorAll('.stab-btn').forEach(b=>{
      b.classList.toggle('on',b.dataset.tab==='sbdash');
    });
  }
  const el=document.getElementById('sbDashContainer');
  if(!el){Toast.show('설정 메뉴를 먼저 열어주세요.','warn');return;}
  ...
  if(typeof _sb!=='undefined'&&_sb){
    ...
    }catch(e){console.warn('[SB] 대시보드 조회 오류',e);}
  }
  tables.forEach(t=>{
  ...
  const EG_USED=0.1, EG_MAX=5;
  const EG_PCT=Math.round((EG_USED/EG_MAX)*100)||1;
  const ST_USED=0.5, ST_MAX=1024;
  const ST_PCT=Math.round((ST_USED/ST_MAX)*100)||1;
  ...
  const sbOk=typeof _sb!=='undefined'&&!!_sb;
  ...
  html+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">'
    +'<span style="font-size:12px;font-weight:600;color:'+(sbOk?'#22c55e':'#ef4444')+'">'
    +(sbOk?'● 연결됨':'● 미연결')+'</span>'
  ...
  html+='<div class="card" style="margin-bottom:12px">'
    ...
    +'<div style="text-align:center"><div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">Database</div>'+donut(DB_PCT,DB_PCT>80?'#ef4444':DB_PCT>50?'#f59e0b':'#3b82f6',DB_USED+'MB','/'+DB_MAX+'MB')+'</div>'
    +'<div style="text-align:center"><div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">Storage</div>'+donut(ST_PCT,'#10b981',ST_USED+'MB','/1GB')+'</div>'
    +'<div style="text-align:center"><div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">Egress</div>'+donut(EG_PCT,'#f59e0b',EG_USED+'GB','/'+EG_MAX+'GB')+'</div>'
    +'<div style="text-align:center"><div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">전체 행</div>'+donut(Math.min(100,Math.round(totalRows/500)),'#8b5cf6',totalRows.toLocaleString(),'rows')+'</div>'
    +'<div style="text-align:center"><div style="font-size:10px;font-weight:600;color:var(--tm);margin-bottom:4px">비활성 방지</div>'+donut(Math.min(100,Math.round((daysLeft/7)*100)),aliveColor,daysLeft+'일','/ 7일')+'</div>'
    +'</div></div>';

  /* 테이블별 현황 */
  html+='<div class="card" style="margin-bottom:12px">'
    +'<div class="ch"><div class="ct">📋 테이블별 현황</div>'
    +'<span style="font-size:11px;color:var(--tm)">총 '+totalRows.toLocaleString()+'행</span></div>'
    +'<table style="width:100%;border-collapse:collapse;font-size:12px">'
    +'<thead><tr style="background:var(--bg2)">'
    +'<th style="padding:5px 10px;text-align:left">그룹</th>'
    +'<th style="padding:5px 10px;text-align:left">테이블</th>'
    +'<th style="padding:5px 10px;text-align:right">행 수</th>'
    +'<th style="padding:5px 10px">비율</th>'
    +'</tr></thead><tbody>'
    +tables.map(t=>{
      const cnt=counts[t.key]||0;
      const pct2=estBytes>0?Math.min(100,Math.round(cnt*(AVG[t.key]||400)/estBytes*100)):0;
      return '<tr style="border-bottom:1px solid var(--bd)">'
        +'<td style="padding:5px 10px;font-size:10px;color:var(--tm)">'+H.e(t.group)+'</td>'
        +'<td style="padding:5px 10px;font-weight:600">'+H.e(t.label)+'</td>'
        +'<td style="padding:5px 10px;text-align:right">'+cnt.toLocaleString()+'</td>'
        +'<td style="padding:5px 10px"><div style="background:var(--bd);border-radius:3px;height:5px">'
  ...
  /* keepalive */
  html+='<div class="card">'
    +'<div class="ch"><div class="ct">🛡️ 비활성 방지 (7일 미접속 시 정지)</div></div>'
  ...
  html+='<div style="text-align:center;padding:16px;color:var(--tl)">"🔄 목록 보기" 버튼을 클릭하면 삭제된 데이터를 확인하고 복구할 수 있습니다.</div>';
  html+='</div></div>';
...
async _sbKeepAlive(){
  /* [v2.372] Supabase keepalive — 7일 비활성 방지 */
  try{
...
/* ════════════════════════════════════════
   검사 고도화 — 검사 성적서 [v2.372]
   ════════════════════════════════════════ */
async insp_cert(){
  /* [v2.372] spin → 레이아웃 없을 때만, 있으면 데이터만 갱신 */
  const w=document.getElementById('pw');
...
/* ════════════════════════════════════════
   검사 고도화 — Hold 관리 [v2.372]
   ════════════════════════════════════════ */
async insp_hold(){
  /* [v2.372] SB holds 로드 — spin 최초1회 hasLayout */
  const w=document.getElementById('pw');
...
/* ════════════════════════════════════════
   검사 고도화 — 재검사 관리 [v2.372]
   ════════════════════════════════════════ */
async insp_reinsp(){
  /* [v2.372] SB reinspections 로드 — spin 최초1회 */
  const w=document.getElementById('pw');
  ...
  Modal.open({
    ...
    body:`<div class="fg2">
      ...
      <div class="fgroup"><label class="fl">원검사번호</label>
        <input class="fc" id="riOrig" value="${isEdit?H.e(row.orig_no||''):''}"></div>
      <div class="fgroup"><label class="fl req">LOT번호</label>
        <input class="fc" id="riLot" value="${isEdit?H.e(row.lot_no||''):''}"></div>
...
async _reinspSave(){
  ...
  const newRow={reinsp_no:no,lot_no:lot,orig_no:orig,req_date:date,
    status:row?.status||'요청',created_by:Auth._u?.name||''};
  if(row&&row.id){
...
/* [v2.372] settings 공지/로고 — Cfg에 실제 구현, Pages에서 위임 */
...
/* ── 검사 기준서 렌더 [v2.372] ── */
_inspStdRender(){
  /* [v2.372] 무한루프 수정 — w.innerHTML 최초1회, 테이블만 갱신 */
  const w=document.getElementById('pw');
  if(!w) return;
...
/* 심사 결과 메일 발송 [v2.372] — Gmail MCP 연동 */
...
const ExcelMgr={
  ...
  _schemas:{
    items:{
      ...
      /* [v2.372] 품목코드만 중복 확인, 필수값 외 빈칸 허용 */
      dupKey:'item_code',
      dupLabel:'품목코드',
    ...
    vendors:{
      ...
      /* [v2.372] 거래처명만 중복 확인, 필수값 외 빈칸 허용 */
      dupKey:'vendor_name',
      dupLabel:'거래처명',
      dupOnly:true,
      getData:()=>DB.vendors,
    ...
    insp_in:{
      ...
      /* [v2.372] 품목코드+거래처명 없으면 등록 안 됨, 동일시트 중복 허용 */
    ...
    insp_pr:{
      ...
      dupKey:'insp_no', dupLabel:'검사번호',
      allowFileDup:true,
      /* [v2.372] 품목코드+거래처명 필수, 동일시트 중복 허용 */
      dupKey:'insp_no', dupLabel:'검사번호',
      allowFileDup:true,
    ...
    nc:{
      /* [v2.372] 부적합 스키마 — 사내외/품목코드/수량/원인/조치/기한 추가 */
      title:'부적합관리',
      cols:[
  ...
  _parseFile(file,page){
    ...
    reader.onload=e=>{
      try{
        const wb=XLSX.read(e.target.result,{type:'array',cellDates:false});
        const ws=wb.Sheets[wb.SheetNames[0]];
        this._ws=ws; /* [v2.372] ws 저장 — 날짜 변환에 사용 */
        const raw=XLSX.utils.sheet_to_json(ws,{header:1,defval:''});
        /* [v2.372] 날짜 필드 변환 — 엑셀 시리얼/Date객체 → YYYY-MM-DD */
  ...
  _renderPreview(raw,page){
    ...
    /* ── 헤더행(0행) 처리 ──
       [v2.372 버그수정] 인덱스 기반 → 헤더 레이블 기반 파싱
  ...
  async _register(page){
    ...
    /* ── items 허용 컬럼 추출 함수 ── */
    const toAllowed=row=>{
      /* [v2.372] 테이블별 허용 컬럼만 추출 — SB schema 오류 방지 */
    ...
    /* [v2.372] equip: SB.addEquip 헬퍼 직접 호출 (검사5종 방식과 동일)
  ...
  /* 멀티시트 양식 다운로드
     [v2.372] pageFilter: 특정 시트만 포함 (null=전체) */
  ...
  /* 멀티시트 업로드 모달
     [v2.372] pageFilter: 특정 시트만 표시 (예: 'vendors', 'insp_in' 등)
             null이면 전체 8개 시트 표시 */
    ...
    reader.onload=async(ev)=>{
      try{
        ...
        for(const [sName,pKey] of Object.entries(SMAP)){
          ...
          /* [v2.372] raw:true 유지 — 셀 직접 접근으로 날짜 변환 처리 */
          const raw=XLSX.utils.sheet_to_json(ws,{header:1,defval:''});
  ...
  async _registerAll(){
    ...
    /* [v2.372] 엑셀 날짜 → YYYY-MM-DD (Date객체/시리얼/문자열 모두 처리) */
    const _toDate=(v)=>{
    ...
    const toFullRow=(pKey,r)=>{
      ...
      /* [v2.372] 검사5종 — SB.addInspection allowed와 동일 컬럼 */
      ...
      /* [v2.372] 계측기 — SB.addEquip allowed와 동일 컬럼 */
    ...
    for(const [pKey,result] of Object.entries(this._multiParsed)){
      ...
      /* [v2.372] 검사5종: SB.addInspection 직접 호출 (거래처 방식 — 컬럼 오류 자동 처리) */
      ...
      /* [v2.372] 계측기: SB.addEquip 직접 호출 (거래처 방식 — 컬럼 오류 자동 처리) */
        ...
        while(att<3&&!ok){
          try{
            ...
            if(!error){totalSuccess+=chunk.length;ok=true;}
              ...
              /* [v2.372] SAFE 저장 성공하면 colErrors 제거 — SQL 실행 후 팝업 억제 */
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.372] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.372 수정] 복원 시 저장된 사용자 정보로 표시 (admin 하드코딩 제거) */
      Auth._cur = cur;
      Auth._u   = u;
      ...
      /* [v2.372] 설정 메뉴: 세션 복원 시에도 admin만 표시 */
      ...
      /* [v2.372] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
