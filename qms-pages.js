/* qms-pages.js — Pages 페이지 렌더러 [v2.366] */
"use strict";
    ...
    <div class="hw-main">
      ...
      <div class="hw-hdr">
        ...
        <div class="hw-hdr-center">
          ...
          <div class="hw-hdr-sub">Quality Management System · v2.366</div>
        </div>
...
async _mentionReplySend(parentId){
  /* [v2.366 PhaseB] thread_id 기반 SB 스레드 저장 */
...
/* [v2.366] perms 저장 — sessionStorage + SB users 테이블 */
_savePerms(){
  try{
...
async _changePw(){
  ...
  /* [v2.366] admin 비밀번호 변경 — sessionStorage 저장 */
...
/* ── 사용자 ── */
async users(){
  /* [v2.366] 사원관리 — 인사 목록 (권한관리 없음, 활성사용자만) */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
...
async _uSave(id){
  ...
  if(id){
    ...
    /* [v2.366] admin 계정 이메일 수정 시 로그인창 footer 반영 */
  ...
  /* [v2.366] 저장 후 사원관리 이동 제거 — 사용자관리 탭 유지 */
  Modal.close();
  /* [v2.366] settings 사용자관리 탭 내 목록 갱신 — window.renderUserMgmt 사용 */
...
_ncForm(row=null){
  /* [v2.366] 부적합 등록/수정 폼 — 사내외/품목코드/고객 유형 추가 */
  const isEdit=!!row;
...
/* 양식 다운로드 */
_equipDownload(){
  /* [v2.366] 단순 양식 생성 — _EQUIP_COLS 직접 사용 */
...
async _equipDoUpload(){
  /* [v2.366] parsed 데이터를 SB.addEquip으로 저장 */
...
async equip(){
  ...
  /* [v2.366] status null인 데이터 실시간 재계산 */
  /* [v2.366] status 항상 재계산 — DB 저장값 무시하고 next 날짜 기준으로 */
  DB.equip.forEach(e=>{
  ...
  Tbl.render({el:'#eqTbl',cols:[
    /* [v2.366] 컬럼 순서: 요청사항 기준 재정의 + model 복구 */
...
cal(){
  const w=document.getElementById('pw');
  /* [v2.366 Phase3] D-30 이내 + 아직 만료 안된 계측기 */
  const _now=new Date();
...
/* [v2.366 P4-5] 교정 주기 기반 차기교정일 자동 계산 */
_calAutoNext(){
...
/* [v2.366 P4-3] 교정비용 연도별 + 계측기별 통계 차트 */
_calCostChart(){
...
async mentions(){
  ...
  const renderList=()=>{
    ...
    const cardHtml=(m)=>{
    /* [v2.366] 멘션 1행 — dt 클래스 방식, 폰트 inherit, 클릭 완전 구현 */
    ...
    if(!listEl) return;
    /* [v2.366] 목록 헤더 */
    /* [v2.366] 멘션 table: width 100%, font inherit(기존 메뉴와 동일) */
    const headerHtml=
...
async _mentionRead(id){
  ...
  m.read=true;
  /* [v2.366 PhaseB] 배지 업데이트 — 내 미읽음 기준 */
...
/* [v2.366 PhaseB] 업무 레코드 바로가기 링크 맵 */
...
/* [v2.366 PhaseB] 스레드 답글 — SB 연동 */
_mentionReplyView(id){
  /* [v2.366] 멘션 상세팝업 — renderBody 제거, 단순 문자열 직접 구성 */
...
/* [v2.366 PhaseC] 미응답 팔로우업 — 3일 경과 미처리 알림 */
_mentionFollowUp(){
  const me=Auth._cur||'admin';
  const isAdmin=Auth._u?.role==='admin';
  const now=new Date();
...
/* ── 설정 ── */
async settings(){
  /* [v2.366] settings 완전 재작성 — 3탭: 일반설정/사용자관리/SB대시보드 */
  const w=document.getElementById('pw');
  ...
  /* ── renderTab ── */
  /* [v2.366] renderTab — settings 내부만, 각 패널 명시적 제어 */
  const renderTab=(tab)=>{
    const pw=document.getElementById('pw');
    if(!pw) return;
    /* 탭 버튼 활성화 */
  ...
  const renderUserMgmt=(sortKey='name',sortDir=1)=>{
    /* [v2.366] 사용자 관리 — 정렬 기능 + 12px + 30px 행높이 */
    const us=DB.users||[];
...
`;
  ...
  /* [v2.366] renderUserMgmt, renderPermTable을 window에 노출 → _uSave에서 접근 가능 */
  window.renderTab=renderTab;
  window.renderUserMgmt=renderUserMgmt;
  window.renderPermTable=renderPermTable;
},
/* [v2.366] 시스템 → 사용자 등록 — 개인정보 및 권한 관리 */
async sysusers(){
  /* [v2.366] 사용자 등록 → settings() 로드 후 usermgmt 탭 즉시 활성화 */
  await Pages.settings();
...
_renderSysUsers(w=null){
  /* [v2.366] 사용자 등록/권한 관리 독립 렌더 */
  /* [v2.366] w가 있으면 레이아웃 생성 */
  if(w){
...
/* [v2.366] SB 대시보드 — 사용량 + 비활성 방지 */
async _renderSbDash(){
  /* [v2.366] SB 대시보드 — 완전 재작성 */
...
async _sbKeepAlive(){
  /* [v2.366] Supabase keepalive — 7일 비활성 방지 */
  try{
...
Object.assign(Pages,{
async insp_std(){
  /* [v2.366] 검사 기준서 — SB 연동, 품목 DB 연동, F2 등록 */
  const w=document.getElementById('pw');
  w.innerHTML='<div class="spin"></div>';
...
/* ── 검사 기준서 렌더 [v2.366] ── */
_inspStdRender(){
  /* [v2.366] 무한루프 수정 — w.innerHTML 최초1회, 테이블만 갱신 */
  const w=document.getElementById('pw');
  if(!w) return;
...
const ExcelMgr={
  ...
  _schemas:{
    ...
    nc:{
      /* [v2.366] 부적합 스키마 — 사내외/품목코드/수량/원인/조치/기한 추가 */
      title:'부적합관리',
      cols:[
...
function setupHotkeys(){
  document.addEventListener('keydown',ev=>{
    if(ev.key==='F2'){
      ...
      /* [v2.366] F5 브라우저 새로고침 방지 — 앱 내부에서 현재 페이지 재렌더 */
      ev.preventDefault();
      if(Auth._u){
...
(function init(){
  ...
  if(savedAuth){
    try{
      ...
      /* [v2.366] DB 일괄 로드 완료 후 페이지 이동 — 빈 DB로 렌더 방지 */
      (async()=>{
        try{
          if(_sb){
